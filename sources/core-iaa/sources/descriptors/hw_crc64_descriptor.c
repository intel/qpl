...
HW_PATH_IAA_API(void, descriptor_init_crc64,
                (hw_descriptor* const descriptor_ptr, const uint8_t* const source_ptr, const uint32_t size,
                 const uint64_t polynomial, const bool is_be_bit_order, const bool is_inverse)) {
    ...
    if (is_be_bit_order) { this_ptr->crc_flags = ADC64F_BE; }

    if (is_inverse) { this_ptr->crc_flags |= ADC64F_INVCRC; }
