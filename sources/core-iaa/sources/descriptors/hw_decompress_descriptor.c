...
HW_PATH_IAA_API(void, descriptor_inflate_set_aecs,
    ...
    if (is_final) { this_ptr->decomp_flags |= ADCF_FLUSH_OUTPUT; }
}
...
HW_PATH_IAA_API(void, descriptor_init_inflate_header,
    ...
    this_ptr->op_code_op_flags =
            ADOF_OPCODE(QPL_OPCODE_DECOMPRESS) | ADOF_WRITE_SRC2(AD_WRSRC2_ALWAYS) | read_flag | toggle_aecs_flag;

    this_ptr->decomp_flags =
            ADDF_ENABLE_DECOMP | ADDF_SUPPRESS_OUTPUT | ADDF_IGNORE_END_BITS(MAX_BIT_IDX & ignore_end_bits);

    this_ptr->src2_ptr  = (uint8_t*)aecs_ptr;
    this_ptr->src2_size = HW_AECS_FILTER_AND_DECOMPRESS_WA_HB;

    if (access_policy & hw_aecs_access_read) {
        hw_iaa_aecs_analytic* read_aecs_ptr =
                (hw_iaa_aecs_analytic*)(this_ptr->src2_ptr + this_ptr->src2_size * (access_policy & hw_aecs_toggle_rw));
...
HW_PATH_IAA_API(void, descriptor_init_inflate_body,
    ...
    this_ptr->decomp_flags = ADDF_ENABLE_DECOMP | ADDF_FLUSH_OUTPUT | ADDF_STOP_ON_EOB |
...
HW_PATH_IAA_API(void, descriptor_init_huffman_only_decompress,
    ...
    this_ptr->trusted_fields   = 0U;
    this_ptr->op_code_op_flags = ADOF_OPCODE(QPL_OPCODE_DECOMPRESS) | ADOF_READ_SRC2(AD_RDSRC2_AECS);
    this_ptr->decomp_flags     = ADDF_ENABLE_DECOMP | ADDF_FLUSH_OUTPUT | (huffman_be ? ADDF_DECOMP_BE : 0U) |
                             ADDF_IGNORE_END_BITS(MAX_BIT_IDX & ignore_end_bits);

    if (!is_gen1) { this_ptr->decomp_flags |= (ignore_end_bits > 7U ? ADDF_IGNORE_END_BITS_EXT : 0U); }
...
HW_PATH_IAA_API(void, descriptor_set_inflate_stop_check_rule,
    ...
    if (check_for_eob) { stop_check_mask |= ADDF_CHECK_FOR_EOB; }
...
HW_PATH_IAA_API(void, descriptor_init_compress_verification, (hw_descriptor * descriptor_ptr)) {
    ...
    this_ptr->op_code_op_flags = ADOF_OPCODE(QPL_OPCODE_DECOMPRESS);
    this_ptr->decomp_flags     = ADDF_ENABLE_DECOMP | ADDF_STOP_ON_EOB | ADDF_SEL_BFINAL_EOB | ADDF_SUPPRESS_OUTPUT;
    this_ptr->filter_flags     = 0U;
    this_ptr->src2_ptr         = NULL;
    this_ptr->src2_size        = 0U;
    this_ptr->dst_ptr          = (uint8_t*)this_ptr;
    this_ptr->max_dst_size     = 1U;
}
...
HW_PATH_IAA_API(void, descriptor_compress_verification_write_initial_index,
    ...
    own_index_t* initial_index_ptr = (own_index_t*)this_ptr->dst_ptr;

    initial_index_ptr->crc        = crc;
    initial_index_ptr->bit_offset = bit_offset;

    this_ptr->dst_ptr += sizeof(own_index_t);
    this_ptr->max_dst_size -= sizeof(own_index_t);
