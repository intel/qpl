...
#define QPL_BADARG_RET(expr, err_code)         \
    {                                          \
        if (expr) { QPL_ERROR_RET(err_code); } \
    }
#else
#define QPL_BADARG_RET(expr, err_code)
#endif

// todo: merge with own_qplc_defs.h
#ifndef OWN_RETURN_ERROR
#define OWN_RETURN_ERROR(expression, error_code) \
    {                                            \
...
#define HW_IMMEDIATELY_RET(expression, status) \
    if ((expression)) { return (qpl_status)(status); }

#define HW_IMMEDIATELY_RET_NULLPTR(expression) \
    if (NULL == (expression)) { return QPL_STS_NULL_PTR_ERR; }

#define QPL_BAD_PTR2_RET(ptr1, ptr2) \
    {                                \
        QPL_BAD_PTR_RET(ptr1);       \
        QPL_BAD_PTR_RET(ptr2)        \
    }

#define QPL_BAD_SIZE_RET(n) QPL_BADARG_RET((n) <= 0, QPL_STS_SIZE_ERR)

#define QPL_VALID_OP                                                                                               \
    ((1ULL << qpl_op_decompress) | (1ULL << qpl_op_compress) | (1ULL << qpl_op_crc64) | (1ULL << qpl_op_extract) | \
     (1ULL << qpl_op_select) | (1ULL << qpl_op_expand) | (1ULL << qpl_op_scan_eq) | (1ULL << qpl_op_scan_ne) |     \
