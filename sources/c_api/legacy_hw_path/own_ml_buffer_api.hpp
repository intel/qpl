...
static inline void own_qpl_buffer_fill(qpl_buffer* const buffer_ptr, uint8_t* const source_ptr,
                                       const uint32_t source_size) {
    qpl::core_sw::util::copy(source_ptr, source_ptr + source_size, buffer_ptr->data + buffer_ptr->actual_bytes);
...
static inline bool own_qpl_buffer_touch(const qpl_buffer* const buffer_ptr, const uint32_t source_size) {
