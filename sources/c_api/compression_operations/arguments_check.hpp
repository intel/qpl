...
inline qpl_status bad_arguments_check<qpl_operation::qpl_op_compress>(const qpl_job *const job_ptr) noexcept {
    ...
    if (job::is_indexing_enabled(job_ptr) && job_ptr->idx_array == nullptr) {
    ...
    } else if (job_ptr->flags & QPL_FLAG_NO_HDRS &&
    ...
    if (job_ptr->level == qpl_high_level &&
        (job::get_execution_path(job_ptr) == ml::execution_path_t::hardware)) {
        return QPL_STS_UNSUPPORTED_COMPRESSION_LEVEL;
    }
