...
static inline void own_deflate_job_update_missed_literals(own_deflate_job* const job_ptr,
    ...
    while (job_ptr->current_ptr < upper_bound_ptr) {
        ...
        own_deflate_hash_table_update(&job_ptr->histogram_ptr->table,
                                      (uint32_t)(job_ptr->current_ptr - job_ptr->lower_bound_ptr), hash_value);
...
void own_deflate_job_perform(own_deflate_job* const job_ptr, const own_deflate_match_finder match_finder,
    ...
    // Main cycle
    while (job_ptr->current_ptr < (upper_bound_ptr - QPLC_DEFLATE_MINIMAL_MATCH_LENGTH) && predicate(job_ptr)) {
        // Variables
        const deflate_match_t longest_match =
                match_finder(&job_ptr->histogram_ptr->table, lower_bound_ptr, job_ptr->current_ptr,
    ...
    if (predicate(job_ptr)) { literals_handler(job_ptr, upper_bound_ptr, true); }
...
void own_initialize_deflate_job(own_deflate_job* const job_ptr, const uint8_t* const source_ptr,
    ...
    if (initial_status == job_ptr->job_status) { bit_writer_init(&job_ptr->bit_writer); }
