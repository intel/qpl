...
QPL_FUN(qpl_status, qpl_gather_deflate_statistics,
    ...
    if (level != qpl_high_level && level != qpl_default_level) { return QPL_STS_UNSUPPORTED_COMPRESSION_LEVEL; }
    ...
    switch (path) {
        case qpl_path_auto:
            status = compression::update_histogram<execution_path_t::hardware>(begin, end, *histogram_ptr);
            // If HW execution returns error, run on SW path
            if (status_list::ok != status) {
                status = compression::update_histogram<execution_path_t::software>(begin, end, *histogram_ptr, level);
            }
        ...
        case qpl_path_software:
            status = compression::update_histogram<execution_path_t::software>(begin, end, *histogram_ptr, level);
            return static_cast<qpl_status>(status);
        default: return QPL_STS_PATH_ERR;
    }
}
}
