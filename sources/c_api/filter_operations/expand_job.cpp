...
uint32_t perform_expand(qpl_job* job_ptr, uint8_t* unpack_buffer_ptr, uint32_t unpack_buffer_size,
    ...
    const auto mask_stream_format =
            job_ptr->flags & QPL_FLAG_SRC2_BE ? stream_format_t::be_format : stream_format_t::le_format;
    const auto output_stream_format =
            (job_ptr->flags & QPL_FLAG_OUT_BE) ? stream_format_t::be_format : stream_format_t::le_format;
    ...
    switch (job_ptr->data_ptr.path) {
        case qpl_path_hardware: {
            ...
            auto mask_stream =
                    analytics::input_stream_t::builder(mask_begin, mask_end)
            ...
            // Configure buffers
            limited_buffer_t source_buffer(unpack_buffer_ptr, unpack_buffer_ptr + unpack_buffer_size,
            ...
            result = call_expand<execution_path_t::hardware>(input_stream, mask_stream, output_stream, source_buffer,
                                                             mask_buffer, output_buffer, job_ptr->numa_id);
            break;
        }
        case qpl_path_auto: {
            auto input_stream = analytics::input_stream_t::builder(src_begin, src_end)
            ...
            result = call_expand<execution_path_t::auto_detect>(input_stream, mask_stream, output_stream, source_buffer,
                                                                mask_buffer, output_buffer, job_ptr->numa_id);
            break;
        }
        case qpl_path_software: {
            auto input_stream = analytics::input_stream_t::builder(src_begin, src_end)
                                        ...
                                        .compressed(job_ptr->flags & QPL_FLAG_DECOMPRESS_ENABLE,
                                                    static_cast<qpl_decomp_end_proc>(job_ptr->decomp_end_processing),
                                                    job_ptr->ignore_end_bits)
                                        .decompress_buffer<execution_path_t::software>(decompress_buffer_begin,
                                                                                       decompress_buffer_end)
                                        .stream_format(input_stream_format, job_ptr->src1_bit_width)
            ...
            result = call_expand<execution_path_t::software>(input_stream, mask_stream, output_stream, source_buffer,
                                                             mask_buffer, output_buffer);
        }
    }

    job_ptr->total_out = result.output_bytes_;

    if (result.status_code_ == 0) { update_job(job_ptr, result); }
