...
uint32_t perform_extract(qpl_job* job_ptr, uint8_t* buffer_ptr, uint32_t buffer_size) {
    ...
    switch (job_ptr->data_ptr.path) {
        case qpl_path_hardware: {
            ...
            extract_result = analytics::call_extract<execution_path_t::hardware>(
        ...
        case qpl_path_auto: {
            ...
            extract_result = analytics::call_extract<execution_path_t::auto_detect>(
                    input_stream, output_stream, job_ptr->param_low, job_ptr->param_high, temporary_buffer,
                    job_ptr->numa_id);
            break;
        }
        case qpl_path_software: {
            ...
            extract_result = analytics::call_extract<execution_path_t::software>(
                    input_stream, output_stream, job_ptr->param_low, job_ptr->param_high, temporary_buffer);
            break;
        }
    }
    ...
    if (extract_result.status_code_ == 0) { update_job(job_ptr, extract_result); }
