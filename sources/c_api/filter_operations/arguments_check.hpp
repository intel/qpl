...
inline qpl_status validate_analytic_buffers(const qpl_job* const job_ptr) noexcept {
    ...
    if constexpr (operation == qpl_op_expand || operation == qpl_op_select) {
        QPL_BAD_PTR_RET(job_ptr->next_src2_ptr)
        QPL_BAD_SIZE_RET(job_ptr->available_src2)

        if (ml::bad_argument::buffers_overlap(job_ptr->next_in_ptr, job_ptr->available_in, job_ptr->next_src2_ptr,
                                              job_ptr->available_src2)) {
            return QPL_STS_BUFFER_OVERLAP_ERR;
        }

        if (ml::bad_argument::buffers_overlap(job_ptr->next_src2_ptr, job_ptr->available_src2, job_ptr->next_out_ptr,
                                              job_ptr->available_out)) {
            return QPL_STS_BUFFER_OVERLAP_ERR;
        }

        if (job_ptr->drop_initial_bytes) { return QPL_STS_DROP_BYTES_ERR; }
...
static inline qpl_status bad_arguments_check(const qpl_job* const job_ptr) {
    ...
    if (job_ptr->parser > qpl_p_parquet_rle) { return QPL_STS_PARSER_ERR; }
...
inline qpl_status bad_arguments_check<qpl_operation::qpl_op_select>(const qpl_job* const job_ptr) noexcept {
    ...
    if (job_ptr->parser != qpl_p_parquet_rle && !(job_ptr->flags & QPL_FLAG_DECOMPRESS_ENABLE)) {
        const uint64_t expected_source_byte_length =
                util::bit_to_byte((uint64_t)job_ptr->num_input_elements * (uint64_t)job_ptr->src1_bit_width);
    ...
    if ((qpl_ow_nom != job_ptr->out_bit_width) && (1U == job_ptr->src1_bit_width)) {
...
inline qpl_status bad_arguments_check<qpl_operation::qpl_op_extract>(const qpl_job* const job_ptr) noexcept {
    if ((qpl_p_parquet_rle != job_ptr->parser) && !(QPL_FLAG_DECOMPRESS_ENABLE & job_ptr->flags)) {
        ...
        if (util::bit_to_byte(input_bits) > (uint64_t)job_ptr->available_in) { return QPL_STS_SRC_IS_SHORT_ERR; }
    }
...
static inline auto validate_input_stream(const input_stream_t& stream, uint32_t low_border_width = 1,
