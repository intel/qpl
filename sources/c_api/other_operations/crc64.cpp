...
uint32_t perform_crc64(qpl_job* const job_ptr) noexcept {
    ...
    switch (qpl::job::get_execution_path(job_ptr)) {
        case execution_path_t::auto_detect:
            result = other::call_crc<execution_path_t::auto_detect>(job_ptr->next_in_ptr, job_ptr->available_in,
                                                                    job_ptr->crc64_poly, is_be_bit_order, is_inverse,
                                                                    job_ptr->numa_id);
            break;
        case execution_path_t::hardware:
            result = other::call_crc<execution_path_t::hardware>(job_ptr->next_in_ptr, job_ptr->available_in,
                                                                 job_ptr->crc64_poly, is_be_bit_order, is_inverse,
                                                                 job_ptr->numa_id);
            break;
        case execution_path_t::software:
            result = other::call_crc<execution_path_t::software>(job_ptr->next_in_ptr, job_ptr->available_in,
                                                                 job_ptr->crc64_poly, is_be_bit_order, is_inverse,
                                                                 job_ptr->numa_id);
            break;
    }
