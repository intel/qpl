...
void qpl_set_long_icf_fg_base(uint8_t* next_in, uint64_t processed, uint64_t input_size,
    ...
    uint32_t dist_start[] = {0x0001, 0x0002, 0x0003, 0x0004, 0x0005, 0x0007, 0x0009, 0x000d, 0x0011, 0x0019, 0x0021,
                             0x0031, 0x0041, 0x0061, 0x0081, 0x00c1, 0x0101, 0x0181, 0x0201, 0x0301, 0x0401, 0x0601,
                             0x0801, 0x0c01, 0x1001, 0x1801, 0x2001, 0x3001, 0x4001, 0x6001, 0x0000, 0x0000};

    if (end_in > end_processed + ISAL_LOOK_AHEAD) end_in = end_processed + ISAL_LOOK_AHEAD;
    ...
    while (next_in < end_processed) {
        ...
        if (len >= 8 + LEN_OFFSET) {
            match_len = compare((next_in + 8) - dist, next_in + 8, end_in - (next_in + 8)) + LEN_OFFSET + 8;

            while (match_len > match_lookup->lit_len && match_len >= LEN_OFFSET + SHORTEST_MATCH) {
                write_deflate_icf(match_lookup, match_len > LEN_MAX ? LEN_MAX : match_len, dist_code, dist_extra);
                match_lookup++;
                next_in++;
                match_len--;
            }
 ...
 ************************************************************
*/
uint64_t qpl_gen_icf_map_h1_base(struct isal_zstream* stream, struct deflate_icf* matches_icf_lookup,
                                 uint64_t input_size) {
 ...
 ************************************************************
*/
static struct deflate_icf* compress_icf_map_g(struct isal_zstream* stream, struct deflate_icf* matches_next,
    ...
    level_buf->icf_buf_avail_out = (icf_buf_end - level_buf->icf_buf_next) * sizeof(struct deflate_icf);
...
static inline void icf_body_next_state(struct isal_zstream* stream) {
    ...
    else if (stream->avail_in <= ISAL_LOOK_AHEAD && (stream->end_of_stream || stream->flush != NO_FLUSH))
...
void qpl_icf_body_hash1_fillgreedy_lazy(struct isal_zstream* stream) {
    ...
    while (matches_next_icf >= matches_end_icf) {
        ...
        if (input_size <= ISAL_LOOK_AHEAD) break;
...
void qpl_isal_deflate_icf_body(struct isal_zstream* stream) {
    switch (stream->level) {
        case 3: qpl_isal_deflate_icf_body_lvl3(stream); break;
        case 2: qpl_isal_deflate_icf_body_lvl2(stream); break;
        case 1:
        default: qpl_isal_deflate_icf_body_lvl1(stream);
    }
}
