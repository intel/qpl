...
static void update_checksum(struct inflate_state* state, uint8_t* start_in, uint64_t length) {
    switch (state->crc_flag) {
        ...
        case ISAL_GZIP_NO_HDR_VER: state->crc = qpl_crc32_gzip_refl(state->crc, start_in, length); break;
        case ISAL_ZLIB:
...
static inline void inflate_in_load(struct inflate_state* state, int min_required) {
    ...
    if (state->read_in_length >= 64) return;
...
static inline int set_and_expand_lit_len_huffcode(struct huff_code* lit_len_huff, uint32_t table_length,
    ...
    if (table_length > LIT_LEN) count[8] -= 2;
    ...
    memcpy(tmp_table, &lit_len_huff[ISAL_DEF_LIT_SYMBOLS], sizeof(*lit_len_huff) * (LIT_LEN - ISAL_DEF_LIT_SYMBOLS));
    memset(&lit_len_huff[ISAL_DEF_LIT_SYMBOLS], 0, sizeof(*lit_len_huff) * (LIT_LEN_ELEMS - ISAL_DEF_LIT_SYMBOLS));
 ...
 * of each code length */
static void make_inflate_huff_code_lit_len(struct inflate_huff_code_large* result, struct huff_code* huff_code_table,
                                           uint32_t table_length, uint16_t* count_total, uint32_t* code_list,
                                           uint32_t multisym) {
    uint32_t  j    = 0U;
    uint16_t  code = 0;
    ...
    if (last_length > ISAL_DECODE_LONG_BITS) last_length = ISAL_DECODE_LONG_BITS + 1;
    ...
    for (; last_length <= ISAL_DECODE_LONG_BITS; last_length++) {
        ...
        memcpy(short_code_lookup + copy_size, short_code_lookup, sizeof(*short_code_lookup) * copy_size);
        copy_size *= 2;
        ...
        if (multisym >= SINGLE_SYM_FLAG || last_length < 2 * min_length) continue;

        /* Encode code pairs */
        for (index1 = count_total[min_length]; index1 < count_total[last_length - min_length + 1]; index1++) {
            sym1_index = code_list[index1];
            sym1       = index_to_sym(sym1_index);
            sym1_len   = huff_code_table[sym1_index].length;
            sym1_code  = huff_code_table[sym1_index].code;
            ...
            for (index2 = count_total[sym2_len]; index2 < count_total[sym2_len + 1]; index2++) {
                ...
                if (sym2 > max_symbol) break;

                sym2_code               = huff_code_table[sym2_index].code;
                code                    = sym1_code | (sym2_code << sym1_len);
                code_length             = sym1_len + sym2_len;
                short_code_lookup[code] = sym1 | (sym2 << 8) | (code_length << LARGE_SHORT_CODE_LEN_OFFSET) |
                                          (2 << LARGE_SYM_COUNT_OFFSET);
            }
        }

        /* Continue if no triples are possible */
        if (multisym >= DOUBLE_SYM_FLAG || last_length < 3 * min_length) continue;

        /* Encode code triples */
        for (index1 = count_total[min_length]; index1 < count_total[last_length - 2 * min_length + 1]; index1++) {
            sym1_index = code_list[index1];
            sym1       = index_to_sym(sym1_index);
            sym1_len   = huff_code_table[sym1_index].length;
            sym1_code  = huff_code_table[sym1_index].code;
            ...
            if (last_length - sym1_len < 2 * min_length) break;

            for (index2 = count_total[min_length]; index2 < count_total[last_length - sym1_len - min_length + 1];
                 index2++) {
                sym2_index = code_list[index2];
                sym2       = index_to_sym(sym2_index);
                ...
                for (index3 = count_total[sym3_len]; index3 < count_total[sym3_len + 1]; index3++) {
                    ...
                    if (sym3 > max_symbol - 1) break;

                    code        = sym1_code | (sym2_code << sym1_len) | (sym3_code << (sym2_len + sym1_len));
    ...
    for (uint32_t i = 0U; i < long_code_length; i++) {
         ...
         * long codes. */
        if (huff_code_table[long_code_list[i]].code_and_extra == INVALID_CODE) continue;

        max_length = huff_code_table[long_code_list[i]].length;
        first_bits = huff_code_table[long_code_list[i]].code_and_extra & ((1 << ISAL_DECODE_LONG_BITS) - 1);

        temp_code_list[0] = long_code_list[i];
        temp_code_length  = 1;

        for (j = i + 1; j < long_code_length; j++) {
            if ((huff_code_table[long_code_list[j]].code & ((1 << ISAL_DECODE_LONG_BITS) - 1)) == first_bits) {
...
static inline void make_inflate_huff_code_dist(struct inflate_huff_code_small* result,
                                               ...
                                               uint16_t* count, uint32_t max_symbol) {
    ...
    uint16_t  long_bits               = 0U;
    uint16_t  min_increment           = 0U;
    uint32_t  code_list[DIST_LEN + 2] = {0U}; /* The +2 is for the extra codes in the static header */
    ...
    uint32_t last_length = huff_code_table[code_list[0]].length;
    if (last_length > ISAL_DECODE_SHORT_BITS) last_length = ISAL_DECODE_SHORT_BITS + 1;
    ...
    for (i = 0; i < long_code_length; i++) {
         ...
         * in the list of long codes and add the current symbol to the list of
         * long codes. */
        if (huff_code_table[long_code_list[i]].code == 0xFFFF) continue;

        max_length = huff_code_table[long_code_list[i]].length;
        first_bits = huff_code_table[long_code_list[i]].code & ((1 << ISAL_DECODE_SHORT_BITS) - 1);

        temp_code_list[0] = long_code_list[i];
        temp_code_length  = 1;

        for (j = i + 1; j < long_code_length; j++) {
            if ((huff_code_table[long_code_list[j]].code & ((1 << ISAL_DECODE_SHORT_BITS) - 1)) == first_bits) {
                max_length                       = huff_code_table[long_code_list[j]].length;
                temp_code_list[temp_code_length] = long_code_list[j];
                temp_code_length++;
            }
        ...
        for (j = 0; j < temp_code_length; j++) {
            ...
            for (; long_bits < (1 << (max_length - ISAL_DECODE_SHORT_BITS)); //NOLINT(bugprone-too-small-loop-variable)
                ...
                result->long_code_lookup[long_code_lookup_length + long_bits] =
                        sym | rfc_lookup_table.dist_extra_bit_count[sym] << DIST_SYM_EXTRA_OFFSET |
                        (code_length << SMALL_LONG_CODE_LEN_OFFSET);
            }
            huff_code_table[sym].code = 0xFFFF;
        }
        result->short_code_lookup[first_bits] =
                long_code_lookup_length | (max_length << SMALL_SHORT_CODE_LEN_OFFSET) | SMALL_FLAG_BIT;
...
static inline void make_inflate_huff_code_header(struct inflate_huff_code_small* result,
    ...
    for (; last_length <= ISAL_DECODE_SHORT_BITS; last_length++) {
        ...
        for (k = count_total[last_length]; k < count_total[last_length + 1]; k++) {
             ...
             * first 9 bits are the code, bits 14:10 are the code length,
             * bit 15 is a flag representing this is a symbol*/
            short_code_lookup[huff_code_table[i].code] = i | (huff_code_table[i].length) << SMALL_SHORT_CODE_LEN_OFFSET;
        }
    }
...
#warning "For best performance, run generate_static_inflate, replace static_inflate.h, and recompile"
    ...
    make_inflate_huff_code_lit_len(&state->lit_huff_code, lit_code, LIT_LEN_ELEMS, lit_count, code_list, multisym);

    if (state->hist_bits && state->hist_bits < 15) max_dist = 2 * state->hist_bits;

    make_inflate_huff_code_dist(&state->dist_huff_code, dist_code, DIST_LEN + 2, dist_count, max_dist);
#endif
#if defined(QPL_LIB)
...
static inline void decode_next_lit_len(uint32_t* next_lits, uint32_t* sym_count, struct inflate_state* state,
                                       struct inflate_huff_code_large* huff_code) {
    uint32_t bit_count = 0U;
    uint32_t bit_mask  = 0U;

    if (state->read_in_length <= ISAL_DEF_MAX_CODE_LEN) inflate_in_load(state, 0);
    ...
    if ((next_sym & LARGE_FLAG_BIT) == 0) {
        ...
        if (bit_count == 0) next_sym = INVALID_SYMBOL;
        ...
        *next_lits = next_sym & LARGE_SHORT_SYM_MASK;
        ...
        next_sym =
                huff_code->long_code_lookup[(next_sym & LARGE_SHORT_SYM_MASK) + (next_bits >> ISAL_DECODE_LONG_BITS)];
...
static inline uint16_t decode_next_dist(struct inflate_state* state, struct inflate_huff_code_small* huff_code) {
    uint32_t bit_count = 0U;
    uint32_t bit_mask  = 0U;
    ...
    if ((next_sym & SMALL_FLAG_BIT) == 0) {
        ...
        next_bits = state->read_in & bit_mask;
        next_sym =
                huff_code->long_code_lookup[(next_sym & SMALL_SHORT_SYM_MASK) + (next_bits >> ISAL_DECODE_SHORT_BITS)];
...
static inline uint16_t decode_next_header(struct inflate_state* state, struct inflate_huff_code_small* huff_code) {
    uint32_t bit_count = 0U;
    uint32_t bit_mask  = 0U;
...
static inline int is_correct_huffman_tree(uint16_t* bit_length_count_table, uint32_t bit_length_count_table_size) {
    ...
    for (uint32_t code_bit_length = 1; code_bit_length <= bit_length_count_table_size; code_bit_length++) {
        ...
        if (accum & QPL_32U_SIGN_BIT_MASK) return 0;
    }
    return 1;
}
#endif
...
static inline int setup_dynamic_header(struct inflate_state* state) {
    ...
    uint16_t          code_count[16], lit_count[MAX_LIT_LEN_COUNT], lit_expand_count[MAX_LIT_LEN_COUNT], dist_count[16];
    uint16_t*         count    = NULL;
    uint16_t          symbol   = 0U;
    ...
    const uint8_t code_length_order[CODE_LEN_CODES] = {0x10, 0x11, 0x12, 0x00, 0x08, 0x07, 0x09, 0x06, 0x0a, 0x05,
                                                       0x0b, 0x04, 0x0c, 0x03, 0x0d, 0x02, 0x0e, 0x01, 0x0f};

    /* If you are given a whole header and it matches the pregen header */
    if (state->avail_in > (hufftables_default.deflate_hdr_count + sizeof(uint64_t)) && header_matches_pregen(state))
    ...
    if (state->disable_multisymbol_lookup_table) { multisym = SINGLE_SYM_FLAG; }
#endif
    ...
    if (state->read_in_length < 14) return ISAL_END_INPUT;
...
#if defined(QPL_LIB)
    if (hlit > 29) return QPL_AD_ERROR_CODE_WRONG_NUM_LL_CODES;
    if (hdist > 29) return QPL_AD_ERROR_CODE_WRONG_NUM_DIST_CODES;
    if (hclen > 15) return QPL_AD_ERROR_CODE_BAD_CL_CODE_LEN;
#else
    if (hlit > 29 || hdist > 29 || hclen > 15) return ISAL_INVALID_BLOCK;
#endif
    ...
    if (state->read_in_length < 0) return ISAL_END_INPUT;

#if defined(QPL_LIB)
    if (!flag) return ISAL_INVALID_BLOCK;
    if (!is_correct_huffman_tree(code_count, QPL_MAX_CL_HUFFMAN_CODE_LEN)) return QPL_AD_ERROR_CODE_BAD_CL_CODE_LEN;
    error_code = set_codes(code_huff, CODE_LEN_CODES, code_count);
    if (error_code) return error_code;
#else
    if (!flag || set_codes(code_huff, CODE_LEN_CODES, code_count)) return ISAL_INVALID_BLOCK;
#endif

    make_inflate_huff_code_header(&inflate_code_huff, code_huff, CODE_LEN_CODES, code_count, CODE_LEN_CODES);
    ...
    while (current < end) {
        ...
        if (state->read_in_length < 0) {
            if (current > &lit_and_dist_huff[256] && lit_and_dist_huff[256].length <= 0) return ISAL_INVALID_BLOCK;
        ...
        if (symbol < 16) {
            ...
            if (count == dist_count) { distanceCounter++; }
            ...
            extra_count = rfc_lookup_table.len_extra_bit_count[previous - LIT_TABLE_SIZE - lit_and_dist_huff];
...
#ifndef QPL_LIB
            if (current + i > end || previous == NULL) return ISAL_INVALID_BLOCK;
#else
            if (previous == NULL) { return QPL_AD_ERROR_CODE_FIRST_LL_CODE_16; }
#endif
            ...
            for (j = 0; j < i; j++) {
                ...
                *current = rep_code;
                ...
                extra_count = rfc_lookup_table.len_extra_bit_count[previous - lit_and_dist_huff - LIT_TABLE_SIZE];
            ...
            if (count == dist_count) { distanceCounter += i; }
            ...
            if (count != dist_count && current > lit_and_dist_huff + LIT_TABLE_SIZE + hlit) {
                ...
                if (current > lit_and_dist_huff + LIT_LEN) previous = current - 1;
            }
...
#ifdef QPL_LIB
    if (hdist + 1U != distanceCounter) { return QPL_AD_ERROR_CODE_WRONG_NUM_DIST_CODES; }
#else
    if (hdist + 1u != distanceCounter) { return ISAL_INVALID_BLOCK; }
#endif

#if defined(QPL_LIB)
    if (current > end) return QPL_AD_ERROR_CODE_WRONG_NUM_DIST_CODES;
    if (lit_and_dist_huff[256].length <= 0) return QPL_AD_ERROR_CODE_NO_LL_CODE;
#else
    if (current > end || lit_and_dist_huff[256].length <= 0) return ISAL_INVALID_BLOCK;
#endif
    ...
    make_inflate_huff_code_dist(&state->dist_huff_code, &lit_and_dist_huff[LIT_LEN], DIST_LEN, dist_count, max_dist);

    if (set_and_expand_lit_len_huffcode(lit_and_dist_huff, LIT_LEN, lit_count, lit_expand_count, code_list))
#if defined(QPL_LIB)
    ...
    make_inflate_huff_code_lit_len(&state->lit_huff_code, lit_and_dist_huff, LIT_LEN_ELEMS, lit_count, code_list,
                                   multisym);
...
#endif
{
    ...
    if (state->read_in_length < 0)
        ...
        if (bytes < 4) return ISAL_END_INPUT;
...
static int qpl_read_header_stateful(struct inflate_state* state) {
    ...
    if (block_state_start == ISAL_BLOCK_HDR) {
        ...
        if (copy_size > state->avail_in) copy_size = state->avail_in;
...
#if defined(QPL_LIB)
    if (ret < QPL_HW_BASE_CODE) return ret;
#endif
...
#if defined(QPL_LIB)
        if (state->tmp_in_size > 0) state->block_state = ISAL_BLOCK_HDR; /* Changing block state */
#else
...
static inline int decode_literal_block(struct inflate_state* state) {
    ...
    if (state->avail_in + bytes == 0 && state->block_state != ISAL_BLOCK_INPUT_DONE) return ISAL_END_INPUT;

    if (state->avail_out == 0 && state->type0_block_len > 0) return ISAL_OUT_OVERFLOW;
        ...
        while (sym_count > 0) {
            ...
            if (next_lit < 256 || sym_count > 1) {
                ...
                *state->next_out = next_lit;
...
#if defined(QPL_LIB)
                if (look_back_dist > IGZIP_HIST_SIZE) { return QPL_AD_ERROR_CODE_BAD_DIST; }
...
#else
                if (state->next_out - look_back_dist < start_out) return ISAL_INVALID_LOOKBACK;
#endif
                ...
                if (state->copy_overflow_length > 0) return ISAL_OUT_OVERFLOW;
            } else
...
static inline uint32_t fixed_size_read(struct inflate_state* state, uint8_t** read_buf, uint32_t read_size) {
...
static inline uint32_t buffer_header_copy(struct inflate_state* state, uint32_t in_len, uint8_t* buf,
                                          uint32_t buffer_len, uint32_t offset, uint32_t buf_error) {
    uint32_t len     = in_len;
    uint32_t buf_len = buffer_len - offset;

    if (len > state->avail_in) len = state->avail_in;
    ...
    if (buf != NULL && buf_len < len) {
        ...
        if (buf != NULL) memcpy(&buf[offset], state->next_in, len);
        state->next_in += len;
        state->avail_in -= len;
...
static inline uint32_t string_header_copy(struct inflate_state* state, char* str_buf, uint32_t str_len, uint32_t offset,
                                          uint32_t str_error) {
    uint32_t max_len = str_len - offset;

    if (max_len > state->avail_in || str_buf == NULL) max_len = state->avail_in;

    uint32_t len = strnlen((char*)state->next_in, max_len);

    if (str_buf != NULL) memcpy(&str_buf[offset], state->next_in, len);
...
#endif
{
    ...
    if (state->read_in_length >= 8 * GZIP_TRAILER_LEN) {
        ...
        if (state->read_in_length >= 8) {
            ...
            store_u64(state->tmp_in_buffer + tmp_in_size, state->read_in >> offset);
            state->read_in        = 0;
            state->read_in_length = 0;
...
int qpl_isal_read_gzip_header(struct inflate_state* state, struct isal_gzip_header* gz_hdr) {
    ...
    switch (block_state) {
        case ISAL_BLOCK_NEW_HDR:
            ...
            if (id1 != 0x1f || id2 != 0x8b) return ISAL_INVALID_WRAPPER;

            if (cm != DEFLATE_METHOD) return ISAL_UNSUPPORTED_METHOD;

            gz_hdr->text = 0;
            if (flags & TEXT_FLAG) gz_hdr->text = 1;
            ...
            if (flags & NAME_FLAG) {
                case ISAL_GZIP_NAME:
                    offset = state->count;
                    ret    = string_header_copy(state, gz_hdr->name, gz_hdr->name_buf_len, offset, ISAL_NAME_OVERFLOW);
                    if (ret) {
            ...
            if (flags & HCRC_FLAG) {
                ...
                case ISAL_GZIP_HCRC:
                    ...
                    if ((hcrc & 0xffff) != load_u16(next_in)) return ISAL_INCORRECT_CHECKSUM;
            }
...
int qpl_isal_read_zlib_header(struct inflate_state* state, struct isal_zlib_header* zlib_hdr) {
    ...
    switch (block_state) {
        case ISAL_BLOCK_NEW_HDR:
            ...
            if (method != DEFLATE_METHOD) return ISAL_UNSUPPORTED_METHOD;

            if ((256 * cmf + flags) % 31 != 0) return ISAL_INCORRECT_CHECKSUM;
...
int qpl_isal_inflate_set_dict(struct inflate_state* state, uint8_t* dict, uint32_t dict_len) {
    ...
    if (state->block_state != ISAL_BLOCK_NEW_HDR || state->tmp_out_processed != state->tmp_out_valid)
...
int qpl_isal_inflate_stateless(struct inflate_state* state) {
    ...
    if (state->crc_flag == IGZIP_GZIP) {
        ...
        if (ret) return ret;
        if (z_hdr.dict_flag) return ISAL_NEED_DICT;
    }
    ...
    while (state->block_state != ISAL_BLOCK_FINISH) {
        ...
        if (ret) break;
        if (state->block_state == ISAL_BLOCK_INPUT_DONE) state->block_state = ISAL_BLOCK_FINISH;
    }
    ...
    if (!ret && state->crc_flag) {
        ...
        switch (state->crc_flag) {
            ...
            case ISAL_ZLIB_NO_HDR: finalize_adler32(state); break;
...
int qpl_isal_inflate(struct inflate_state* state) {
    ...
    if (state->block_state != ISAL_BLOCK_FINISH) {
        ...
        if (state->tmp_out_valid < 2 * ISAL_DEF_HIST_SIZE) {
            ...
            state->avail_out = sizeof(state->tmp_out_buffer) - ISAL_LOOK_AHEAD - state->tmp_out_valid;

            if ((int32_t)state->avail_out < 0) state->avail_out = 0;

            /* Decode into internal buffer until exit */
            while (state->block_state != ISAL_BLOCK_INPUT_DONE) {
                if (state->block_state == ISAL_BLOCK_NEW_HDR || state->block_state == ISAL_BLOCK_HDR) {
                ...
                if ((state->block_state == ISAL_BLOCK_NEW_HDR) &&
                    ((state->decomp_end_proc == DECOMP_STOP_AND_CHECK_FOR_ANY_EOB) ||
                     (state->decomp_end_proc == DECOMP_STOP_ON_ANY_EOB))) {
                    qpl_break = 1;
                    break;
                }
                ...
                if ((state->block_state == ISAL_BLOCK_INPUT_DONE) &&
                    ((state->decomp_end_proc == DECOMP_DONT_STOP_OR_CHECK) ||
                     (state->decomp_end_proc == DECOMP_CHECK_FOR_ANY_EOB)) &&
                    state->avail_in > 0) {
                    state->block_state = ISAL_BLOCK_NEW_HDR;
                    continue;
                }
            ...
            if (state->copy_overflow_length != 0) {
                byte_copy(state->next_out, state->copy_overflow_distance, state->copy_overflow_length);
        ...
        if (copy_size > avail_out) copy_size = avail_out;
        ...
        if (ret == ISAL_INVALID_LOOKBACK || ret == ISAL_INVALID_BLOCK || ret == ISAL_INVALID_SYMBOL) {
            ...
            state->total_out -= state->tmp_out_valid - state->tmp_out_processed;
            if (state->crc_flag) update_checksum(state, start_out, state->next_out - start_out);
            return ret;
        }
        ...
        if (state->block_state != ISAL_BLOCK_INPUT_DONE ||
            state->copy_overflow_length + state->write_overflow_len + (uint32_t)state->tmp_out_valid >
            ...
            if (state->tmp_out_valid == state->tmp_out_processed &&
                ...
                memcpy(state->tmp_out_buffer, state->next_out - ISAL_DEF_HIST_SIZE, ISAL_DEF_HIST_SIZE);
                ...
                if (shift_size > state->tmp_out_processed) shift_size = state->tmp_out_processed;

                memmove(state->tmp_out_buffer, &state->tmp_out_buffer[shift_size], state->tmp_out_valid - shift_size);
        ...
        if (state->write_overflow_len != 0) {
            store_u32(&state->tmp_out_buffer[state->tmp_out_valid], state->write_overflow_lits);
        ...
        if (state->copy_overflow_length != 0) {
            byte_copy(&state->tmp_out_buffer[state->tmp_out_valid], state->copy_overflow_distance,
        ...
        if (state->block_state == ISAL_BLOCK_INPUT_DONE && state->tmp_out_valid == state->tmp_out_processed) {
            state->block_state = ISAL_BLOCK_FINISH;
