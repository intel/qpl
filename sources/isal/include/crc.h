 ...
 * in reflection (or reverse). In ISA-L CRC, function qpl_crc32_ieee is
 * actually designed for normal CRC32 IEEE version. And function
 * qpl_crc32_gzip_refl is actually designed for reflected CRC32 IEEE.
 ...
 * a check whether qpl_crc32_gzip_refl is right one for you instead of
 * qpl_crc32_ieee.
 *
 * @returns 32 bit CRC
 */

uint32_t qpl_crc32_ieee(uint32_t             init_crc, //!< initial CRC value, 32 bits
                        const unsigned char* buf,      //!< buffer to calculate CRC on
                        uint64_t             len       //!< buffer length in bytes (64-bit data)
);
 ...
 * @returns 32 bit CRC
 */
uint32_t qpl_crc32_gzip_refl(uint32_t             init_crc, //!< initial CRC value, 32 bits
                             const unsigned char* buf,      //!< buffer to calculate CRC on
                             uint64_t             len       //!< buffer length in bytes (64-bit data)
);
 ...
 * @returns 32 bit CRC
 */
unsigned int qpl_crc32_iscsi(const unsigned char* buffer,  //!< buffer to calculate CRC on
 ...
 * @returns 32 bit CRC
 */
unsigned int qpl_crc32_iscsi_base(unsigned char* buffer,  //!< buffer to calculate CRC on
                                  int            len,     //!< buffer length in bytes
 ...
 * @returns 16 bit CRC
 */
uint16_t qpl_crc16_t10dif_base(uint16_t seed, //!< initial CRC value, 16 bits
 ...
 * @returns 16 bit CRC
 */
uint16_t qpl_crc16_t10dif_copy_base(uint16_t init_crc, //!< initial CRC value, 16 bits
                                    uint8_t* dst,      //!< buffer destination for copy
                                    uint8_t* src,      //!< buffer source to crc + copy
                                    uint64_t len       //!< buffer length in bytes (64-bit data)
);
 ...
 * @returns 32 bit CRC
 */
uint32_t qpl_crc32_ieee_base(uint32_t seed, //!< initial CRC value, 32 bits
                             uint8_t* buf,  //!< buffer to calculate CRC on
                             uint64_t len   //!< buffer length in bytes (64-bit data)
);
 ...
 * @returns 32 bit CRC
 */
uint32_t qpl_crc32_gzip_refl_base(uint32_t seed, //!< initial CRC value, 32 bits
                                  uint8_t* buf,  //!< buffer to calculate CRC on
                                  uint64_t len   //!< buffer length in bytes (64-bit data)
);
