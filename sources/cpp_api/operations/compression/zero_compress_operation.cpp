...
auto execute(zero_compress_operation &operation, int32_t numa_id) -> execution_result<uint32_t, sync> {
    ...
    if (operation.input_format_ == zero_input_format::word_16_bit) {
        if (available_in & 1) {    // if available_in isn't even -> some elem is incomplete
            return execution_result<uint32_t, sync>(QPL_STS_SRC_IS_SHORT_ERR, 0);
        } else if (available_out < 8) {
            return execution_result<uint32_t, sync>(QPL_STS_DST_IS_SHORT_ERR, 0);
        }
    } else {
        if (available_in & 3) {    // if available_in isn't multiple of 4 -> some elem is incomplete
            return execution_result<uint32_t, sync>(QPL_STS_SRC_IS_SHORT_ERR, 0);
        } else if (available_out < 4) {
            return execution_result<uint32_t, sync>(QPL_STS_DST_IS_SHORT_ERR, 0);
        }
    }
    ...
    auto zero_compress_result = call_zero_operation<actual_path,
                                                    zero_operation_type::compress>(input_format,
                                                                                   source_buffer,
                                                                                   destination_buffer,
                                                                                   crc_type,
                                                                                   numa_id);

    return execution_result<uint32_t, sync>(zero_compress_result.status_code_, zero_compress_result.output_bytes_);
}

template
auto execute<execution_path::software>(zero_compress_operation &operation,
                                       int32_t numa_id) -> execution_result<uint32_t, sync>;

template
auto execute<execution_path::hardware>(zero_compress_operation &operation,
                                       int32_t numa_id) -> execution_result<uint32_t, sync>;

template
auto execute<execution_path::auto_detect>(zero_compress_operation &operation,
                                          int32_t numa_id) -> execution_result<uint32_t, sync>;

} // namespace internal

void zero_compress_operation::set_job_buffer(uint8_t * /* buffer */) noexcept {
