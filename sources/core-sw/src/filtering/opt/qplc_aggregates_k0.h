...
OWN_OPT_FUN(void, k0_qplc_bit_aggregates_8u,
    ...
    *index_ptr += length;
...

    {
        ...
        if (msk64) idx -= (1 + (uint32_t)_lzcnt_u64((uint64_t)msk64));
        if ((int32_t)idx < (int32_t)0) return;
...
OWN_OPT_FUN(void, k0_qplc_aggregates_8u,
            (const uint8_t* src_ptr, uint32_t length, uint32_t* min_value_ptr, uint32_t* max_value_ptr,
             uint32_t* sum_ptr)) {
    ...
    if (0 == min_value) {
        if (max_value >= 0xff) {
            ...
            if (remind) {
                ...
                z_data = _mm512_sad_epu8(
                        z_zero, _mm512_maskz_loadu_epi8(
                                        msk64, (void const*)(src_ptr + length))); /* z_data = s7 s6 s5 s4 s3 s2 s1 s0 */
                z_sum = _mm512_add_epi32(z_sum, z_data);                          /* z_sum  = S7 S6 S5 S4 S3 S2 S1 S0 */
            }
        } else {
            for (uint32_t idx = 0U; idx < length; idx += 64) {
...
OWN_OPT_FUN(void, k0_qplc_aggregates_16u,
            (const uint8_t* src_ptr, uint32_t length, uint32_t* min_value_ptr, uint32_t* max_value_ptr,
             uint32_t* sum_ptr)) {
...
OWN_OPT_FUN(void, k0_qplc_aggregates_32u,
            (const uint8_t* src_ptr, uint32_t length, uint32_t* min_value_ptr, uint32_t* max_value_ptr,
             uint32_t* sum_ptr)) {
