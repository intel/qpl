...
OWN_OPT_FUN(uint32_t, k0_qplc_select_8u,
...
OWN_OPT_FUN(uint32_t, k0_qplc_select_16u,
            (const uint8_t* src_ptr, const uint8_t* src2_ptr, uint8_t* dst_ptr, uint32_t length)) {
        ...
        for (uint32_t idx_inloop = length; (msk != 0); idx_inloop += 16, msk = (__mmask64)((uint64_t)msk >> 16U)) {
            ...
            if (msk16 != 0) {
                z_data = _mm512_cvtepu16_epi32(
                        _mm256_maskz_loadu_epi16(msk16, (__m256i const*)(src_16u_ptr + idx_inloop)));
                z_data   = _mm512_maskz_compress_epi32(msk16, z_data);
                num_data = (uint32_t)_mm_popcnt_u32((uint32_t)msk16);
                y_data   = _mm512_cvtepi32_epi16(z_data);
                msk16    = _bzhi_u32(0xffff, num_data);
...
OWN_OPT_FUN(uint32_t, k0_qplc_select_32u,
            (const uint8_t* src_ptr, const uint8_t* src2_ptr, uint8_t* dst_ptr, uint32_t length)) {
    ...
    for (uint32_t idx = 0U; idx < length; idx += 64) {
        ...
        for (uint32_t idx_inloop = idx; (msk != 0); idx_inloop += 16, msk = (__mmask64)((uint64_t)msk >> 16U)) {
            msk16 = (__mmask16)msk;
            if (msk16 != 0) {
                z_data   = _mm512_maskz_compress_epi32(msk16,
                                                       _mm512_loadu_si512((__m512i const*)(src_32u_ptr + idx_inloop)));
                num_data = (uint32_t)_mm_popcnt_u32((uint32_t)msk16);
                msk16    = _bzhi_u32(0xffff, num_data);
    ...
    if (remind) {
        ...
        for (uint32_t idx_inloop = length; (msk != 0); idx_inloop += 16, msk = (__mmask64)((uint64_t)msk >> 16U)) {
            msk16 = (__mmask16)msk;
            if (msk16 != 0) {
                z_data = _mm512_maskz_compress_epi32(
                        msk16, _mm512_maskz_loadu_epi32(msk16, (__m512i const*)(src_32u_ptr + idx_inloop)));
                num_data = (uint32_t)_mm_popcnt_u32((uint32_t)msk16);
                msk16    = _bzhi_u32(0xffff, num_data);
                _mm512_mask_storeu_epi32((void*)(dst_32u_ptr + selected), msk16, z_data);
                selected += num_data;
