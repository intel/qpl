...
auto split_elements(uint32_t number_of_elements) noexcept -> uint32_t {
    ...
    const uint32_t elements_in_part =
            ml::util::round_to_nearest_multiple(number_of_elements / splitting_factor, ml::byte_bits_size);
...
void split_descriptors<qpl_operation::qpl_op_scan_eq, 8>(hw_descriptor&                reference_descriptor,
    ...
    for (uint32_t i = 0; i < number_of_descriptors - 1; i++) {
        ...
        hw_iaa_descriptor_set_input_buffer(&descriptors[i], current_source_ptr, source_part_size);

        hw_iaa_descriptor_set_number_of_elements(&descriptors[i], part_element_count);

        hw_iaa_descriptor_set_output_buffer(&descriptors[i], current_destination_ptr, destination_part_size);
    ...
    hw_iaa_descriptor_set_input_buffer(&descriptors.back(), current_source_ptr, last_source_part_size);

    hw_iaa_descriptor_set_number_of_elements(&descriptors.back(), last_part_element_count);

    hw_iaa_descriptor_set_output_buffer(&descriptors.back(), current_destination_ptr, last_destination_part_size);
}
...
auto is_operation_splittable(const input_stream_t&                                    input_stream,
    ...
    if (!configuration_supported) { return false; }

    if (input_stream.is_compressed() || input_stream.stream_format() == stream_format_t::prle_format) { return false; }

    if (input_stream.source_size() <= 32_kb) { return false; }

    if (!input_stream.are_aggregates_disabled() || !input_stream.is_checksum_disabled()) { return false; }

    if (output_stream.output_bit_width_format() != output_bit_width_format_t::same_as_input) { return false; }
