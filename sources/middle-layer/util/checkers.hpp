...
#define OWN_QPL_CHECK_STATUS(status_expression)           \
    {                                                     \
        auto value = (status_expression);                 \
        if (value) return static_cast<qpl_status>(value); \
    }

namespace qpl::ml::bad_argument {
static inline auto buffers_overlap(const uint8_t* const ptr1, const size_t ptr1_size, const uint8_t* const ptr2,
                                   const size_t ptr2_size) {
    if ((ptr2 >= ptr1) && (ptr2 < (ptr1 + ptr1_size))) { return true; }

    if ((ptr1 >= ptr2) && (ptr1 < (ptr2 + ptr2_size))) { return true; }
