...
constexpr auto build_deflate_implementation(compression_level_t level, compression_mode_t mode,
    ...
    if (dictionary_support == dictionary_support_t::enabled) {
        if (mode == dynamic_mode) return deflate_dictionary_implementation<dynamic_mode>::instance;
        if (mode == fixed_mode) return deflate_dictionary_implementation<fixed_mode>::instance;
        if (mode == canned_mode) return deflate_dictionary_implementation<canned_mode>::instance;
        return deflate_dictionary_implementation<static_mode>::instance;
    }

    if (mode == dynamic_mode) {
        return level == default_level ? deflate_implementation<default_level, dynamic_mode, block_type>::instance
                                      : deflate_implementation<high_level, dynamic_mode, block_type>::instance;
    }

    return level == default_level ? deflate_implementation<default_level, static_mode, block_type>::instance
...
constexpr auto build_deflate_by_mini_blocks_implementation(compression_mode_t mode)
        -> implementation<deflate_state<execution_path_t::software>> {
    if (mode == dynamic_mode) { return deflate_by_mini_blocks_implementation<dynamic_mode>::instance; }
...
constexpr auto build_implementation(compression_level_t level, compression_mode_t mode,
                                    mini_blocks_support_t mini_blocks_supprt, dictionary_support_t dictionary_support)
        -> implementation<deflate_state<execution_path_t::software>> {
