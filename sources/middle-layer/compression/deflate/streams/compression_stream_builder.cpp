...
auto deflate_state_builder<execution_path_t::software>::output(uint8_t* buffer_ptr, uint32_t buffer_size) noexcept
        -> common_type& {
...
auto deflate_state_builder<execution_path_t::software>::set_isal_internal_buffers(
        uint8_t* const level_buffer_ptr, const uint32_t level_buffer_size, isal_hufftables* const huffman_tables_ptr,
...
auto deflate_state_builder<execution_path_t::software>::compression_level(compression_level_t level) noexcept
        -> common_type& {
...
auto deflate_state_builder<execution_path_t::software>::start_new_block(bool UNREFERENCED_PARAMETER(value)) noexcept
        -> common_type& {
...
void deflate_state_builder<execution_path_t::software>::set_huffman_table(
        qpl_compression_huffman_table* huffman_table) noexcept {
    stream_.isal_stream_ptr_->hufftables = stream_.isal_huffman_table_ptr_;
    ...
    core_sw::util::copy(isal_huffman_table, isal_huffman_table + sizeof(isal_hufftables),
                        reinterpret_cast<uint8_t*>(stream_.isal_stream_ptr_->hufftables));
}

auto deflate_state_builder<execution_path_t::software>::load_current_position(uint32_t total_bytes_written) noexcept
        -> common_type& {
...
void deflate_state_builder<execution_path_t::software>::init() noexcept {
    ...
    if (stream_.mini_blocks_support() == mini_blocks_support_t::enabled && stream_.compression_mode_ == dynamic_mode) {
...
auto deflate_state_builder<execution_path_t::software>::dictionary(qpl_dictionary& dictionary) noexcept
        -> common_type& {
    qpl_isal_deflate_set_dict(stream_.isal_stream_ptr_, get_dictionary_data(dictionary),
