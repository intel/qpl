...
static inline void isal_histogram_get_statistics(const isal_histogram *isal_histogram_ptr,
    ...
    std::copy(isal_histogram_ptr->lit_len_histogram, isal_histogram_ptr->lit_len_histogram + QPLC_DEFLATE_LL_TABLE_SIZE, literal_length_histogram_ptr);

    std::copy(isal_histogram_ptr->dist_histogram, isal_histogram_ptr->dist_histogram + QPLC_DEFLATE_D_TABLE_SIZE, offsets_histogram_ptr);
}
