...
huffman_table_icf::huffman_table_icf(hufftables_icf* huffman_table_ptr) noexcept : huffman_table_(huffman_table_ptr) {}
...
auto write_huffman_table_icf(BitBuf2* bit_buffer, huffman_table_icf& huffman_table, isal_mod_hist* histogram,
    ...
    if (compression_mode != fixed_mode) {
        ...
        for (; i < huffman_table.max_ll_code_index_ + 1; i++) {
            combined_table[i] = ll_codes[i].length;
            compressed_len +=
                    (ll_codes[i].length + length_code_extra_bits[i - 257]) * static_cast<uint64_t>(ll_histogram[i]);
            fixed_compressed_len += (fixed_ll_codes[i].length + length_code_extra_bits[i - 257]) *
                                    static_cast<uint64_t>(ll_histogram[i]);
        }
        ...
        for (i = 0; i < huffman_table.max_d_code_index_ + 1; i++) {
            ...
            fixed_compressed_len +=
                    (fixed_d_codes[i].length + distance_code_extra_bits[i]) * static_cast<uint64_t>(d_histogram[i]);
        }
    } else {
    ...
    if (fixed_compressed_len > compressed_len && compression_mode != fixed_mode) {
        ...
        num_cl_tokens = rl_encode(combined_table, huffman_table.max_ll_code_index_ + 1, cl_counts, cl_tokens);

        // Perform RLE for distance length codes
        num_cl_tokens += rl_encode(combined_table + huffman_table.max_ll_code_index_ + 1,
                                   huffman_table.max_d_code_index_ + 1, cl_counts, cl_tokens + num_cl_tokens);

        /* Create header */
        create_header(bit_buffer, cl_tokens, num_cl_tokens, cl_counts,
        ...
        core_sw::util::copy(fixed_hufftables_ptr, fixed_hufftables_ptr + sizeof(hufftables_icf),
...
void prepare_histogram(isal_mod_hist* histogram) noexcept {
    ...
    if (histogram->ll_hist[end_of_block_code_index] == 0) { histogram->ll_hist[end_of_block_code_index] = 1; }
}
