...
static inline qplc_slow_deflate_icf_body_t_ptr qplc_slow_deflate_icf_body() {
    return (qplc_slow_deflate_icf_body_t_ptr)(qpl::core_sw::dispatcher::kernels_dispatcher::get_instance()
                                                      .get_deflate_table()[0]);
}

namespace qpl::ml::compression {

auto write_buffered_icf_header(deflate_state<execution_path_t::software>& stream, compression_state_t& state) noexcept
        -> qpl_ml_status {
...
auto create_icf_block_header(deflate_state<execution_path_t::software>& stream, compression_state_t& state) noexcept
        -> qpl_ml_status {
    auto isal_state   = &stream.isal_stream_ptr_->internal_state;
    auto bit_buffer   = &isal_state->bitbuf;
    auto level_buffer = reinterpret_cast<level_buf*>(stream.isal_stream_ptr_->level_buf);
    ...
    uint64_t block_size =
            stored_block_header_length * ((block_in_size + stored_block_max_length - 1) / stored_block_max_length) +
            block_in_size;
    ...
    const uint64_t bit_count = write_huffman_table_icf(bit_buffer, stream.huffman_table_icf_, &level_buffer->hist,
                                                       stream.compression_mode(), isal_state->has_eob_hdr);
    ...
    if ((bit_count / byte_bit_size >= block_size ||
        ...
        block_start >= stream.source_begin() && block_size <= avail_output) {
        ...
        core_sw::util::copy(bit_writer_tmp_ptr, bit_writer_tmp_ptr + sizeof(BitBuf2),
...
auto flush_icf_block(deflate_state<execution_path_t::software>& stream, compression_state_t& state) noexcept
        -> qpl_ml_status {
    auto isal_state   = &stream.isal_stream_ptr_->internal_state;
    auto bit_buffer   = &isal_state->bitbuf;
    auto level_buffer = reinterpret_cast<level_buf*>(stream.isal_stream_ptr_->level_buf);
    ...
    deflate_icf* icf_buf_encoded_next =
            qpl_encode_deflate_icf(level_buffer->icf_buf_start + isal_state->count, level_buffer->icf_buf_next,
                                   bit_buffer, stream.huffman_table_icf_.get_isal_huffman_tables());
...
auto init_new_icf_block(deflate_state<execution_path_t::software>& stream, compression_state_t& state) noexcept
        -> qpl_ml_status {
    auto isal_state   = &stream.isal_stream_ptr_->internal_state;
    auto level_buffer = reinterpret_cast<level_buf*>(stream.isal_stream_ptr_->level_buf);
...
auto deflate_icf_finish(deflate_state<execution_path_t::software>& stream, compression_state_t& state) noexcept
        -> qpl_ml_status {
    auto isal_state = &stream.isal_stream_ptr_->internal_state;
...
auto deflate_icf_body(deflate_state<execution_path_t::software>& stream, compression_state_t& state) noexcept
        -> qpl_ml_status {
    auto isal_state = &stream.isal_stream_ptr_->internal_state;
...
auto slow_deflate_icf_body(deflate_state<execution_path_t::software>& stream, compression_state_t& state) noexcept
        -> qpl_ml_status {
    auto level_buffer = reinterpret_cast<level_buf*>(stream.isal_stream_ptr_->level_buf);
    ...
    const uint32_t bytes_processed = qplc_slow_deflate_icf_body()(
    ...
    level_buffer->icf_buf_avail_out -=
            static_cast<uint32_t>(icf_stream.next_ptr - icf_stream.begin_ptr) * sizeof(deflate_icf);
