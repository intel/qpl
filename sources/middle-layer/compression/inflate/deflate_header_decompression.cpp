...
auto read_header_stateful(isal_inflate_state& inflate_state) noexcept -> qpl_ml_status {
    ...
    if (ISAL_BLOCK_HDR == block_state_start) {
        ...
        if (copy_size > inflate_state.avail_in) { copy_size = inflate_state.avail_in; }

        core_sw::util::copy(inflate_state.next_in, inflate_state.next_in + copy_size,
    ...
    if (status > status_list::hardware_error_base) { return status; }

    if (ISAL_BLOCK_HDR == block_state_start) {
        /* Setup so state is restored to a valid state */
        auto bytes_read =
                static_cast<int>(inflate_state.next_in - inflate_state.tmp_in_buffer - inflate_state.tmp_in_size);
    ...
    if (status_list::input_too_small == status) {
        ...
        core_sw::util::copy(next_in_start_ptr, next_in_start_ptr + avail_in_start,
                            &inflate_state.tmp_in_buffer[inflate_state.tmp_in_size]);
        ...
        if (inflate_state.tmp_in_size > 0) { inflate_state.block_state = ISAL_BLOCK_HDR; /* Changing block state */ }
    } else {
