...
constexpr uint32_t software_hash_table_size[] = {16 * qpl_1k, 16 * qpl_1k, 64 * qpl_1k, 64 * qpl_1k, 256 * qpl_1k,
                                                 32 * qpl_1k, 32 * qpl_1k, 32 * qpl_1k, 32 * qpl_1k, 32 * qpl_1k};
...
static const uint32_t hash_vals[24] = {0x001, 0x002, 0x004, 0x008, 0x010, 0x020, 0x040, 0x080,
...
static inline void init_hw_dict_hash_table(qpl_dictionary& dictionary) {
    ...
    if (ptrs_per_entry == 2U) {
        ...
        for (uint32_t i = 0; i < 1024; i++) {
            entry        = hash_tbl[i];
            entries32[i] = ((entry >> 0) & 0x0000000F) | ((entry >> 12) & 0x000000F0) | ((entry << 4) & 0x000FFF00) |
                           ((entry << 0) & 0xFFF00000);
        }
    } else {
        // ptrs_per_entry == 4
        ...
        for (uint32_t i = 0; i < 1024; i++) {
            entry        = hash_tbl[i];
            entries64[i] = ((entry >> 0) & 0x000000000000000F) | ((entry >> 12) & 0x00000000000000F0) |
                           ((entry >> 24) & 0x0000000000000F00) | ((entry >> 36) & 0x000000000000F000) |
                           ((entry << 12) & 0x000000000FFF0000) | ((entry << 8) & 0x000000FFF0000000) |
                           ((entry << 4) & 0x000FFF0000000000) | ((entry << 0) & 0xFFF0000000000000);
        }
    }
}

/** @brief Convert the public enum @ref hw_compression_level to internal enum @ref hardware_dictionary_level */
auto convert_public_hw_dict_level_to_internal(hw_compression_level hw_dict_level) noexcept
        -> hardware_dictionary_level {
...
auto get_dictionary_size_in_aecs(qpl_dictionary& dictionary) noexcept -> uint32_t {
    if (hardware_dictionary_level::HW_NONE == dictionary.hw_dict_level) { return 0U; }
...
auto build_dictionary(qpl_dictionary& dictionary, software_compression_level sw_level,
    ...
    core_sw::util::copy(raw_dict_ptr, raw_dict_ptr + raw_dict_size, get_dictionary_data(dictionary));
