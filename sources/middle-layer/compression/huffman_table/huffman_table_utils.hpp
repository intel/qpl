...
struct qpl_compression_huffman_table {
    ...
    */
    std::aligned_storage_t<sizeof(qplc_huffman_table_default_format), qpl::ml::util::default_alignment>
            sw_compression_table_data;

    /**
    * Buffer that contains ISA-L representation of the software compression table
    */
    std::aligned_storage_t<sizeof(isal_hufftables), qpl::ml::util::default_alignment> isal_compression_table_data;
    ...
    */
    std::aligned_storage_t<sizeof(qpl::ml::compression::hw_compression_huffman_table), qpl::ml::util::default_alignment>
            hw_compression_table_data;

    /**
    * Buffer that contains information about deflate header
    */
    std::aligned_storage_t<sizeof(qpl::ml::compression::deflate_header), qpl::ml::util::default_alignment>
...
struct qpl_decompression_huffman_table {
    ...
    */
    std::aligned_storage_t<sizeof(qplc_huffman_table_flat_format), qpl::ml::util::default_alignment> sw_flattened_table;

    /**
    * Buffer that contains representation of the hardware compression table
    * @note currently this is just a stab, this field is not actually used anywhere
    */
    std::aligned_storage_t<sizeof(qpl::ml::compression::hw_decompression_state), HW_PATH_STRUCTURES_REQUIRED_ALIGN>
            hw_decompression_state;
    ...
    */
    std::aligned_storage_t<sizeof(qpl::ml::compression::canned_table), qpl::ml::util::default_alignment>
            lookup_table_buffer;
};
...
auto huffman_table_init(table_t& table, const qpl_triplet* const triplets_ptr, const std::size_t triplets_count,
                        const uint32_t representation_flags = 0U) noexcept -> qpl_ml_status;

template <class table_t>
auto huffman_table_init(table_t& table, const uint32_t* literals_lengths_histogram_ptr,
                        const uint32_t* distances_histogram_ptr, const uint32_t representation_flags = 0U) noexcept
        -> qpl_ml_status;

template <class table_t>
auto huffman_table_init_with_stream(table_t& table, const uint8_t* const buffer,
                                    const uint32_t representation_flags) noexcept -> qpl_ml_status;

template <class first_table_t, class second_table_t>
auto huffman_table_convert(const first_table_t& first_table, second_table_t& second_table,
                           const uint32_t representation_flags = 0U) noexcept -> qpl_ml_status;

template <class table_t>
auto huffman_table_write_to_stream(const table_t& table, uint8_t* const buffer,
                                   const uint32_t representation_flags) noexcept -> qpl_ml_status;
...
static inline auto get_path_flags(execution_path_t path) {
    switch (path) {
        case execution_path_t::hardware: return QPL_HW_REPRESENTATION;
        case execution_path_t::software: return QPL_SW_REPRESENTATION;
        default: return QPL_SW_AND_HW_COMPRESSION_TABLES;
    }
}
