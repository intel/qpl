...
template <>
auto input_stream_t::unpack<analytic_pipeline::prle>(limited_buffer_t& output_buffer, size_t required_elements) noexcept
        -> unpack_result_t {
    ...
    auto status = unpack_prle_kernel_(&current_source_ptr_, current_source_size_, bit_width_, &current_ptr,
                                      static_cast<uint32_t>(required_elements), &prle_count_, &prle_value_);
    ...
    elements_processed =
            (current_number_of_elements_ < elements_processed) ? current_number_of_elements_ : elements_processed;
...
auto input_stream_t::unpack<analytic_pipeline::inflate>(limited_buffer_t& output_buffer,
    ...
    if (elements_to_decompress <= current_number_of_elements_) { state_.terminate(); }
    ...
    if (result.status_code_ == status_list::more_output_needed) { result.status_code_ = status_list::ok; }

    return unpack_result_t(result.status_code_, elements_to_unpack, unpacked_bytes);
}

template <>
auto input_stream_t::unpack<analytic_pipeline::inflate>(limited_buffer_t& output_buffer) noexcept -> unpack_result_t {
...
auto input_stream_t::unpack<analytic_pipeline::inflate_prle>(limited_buffer_t& output_buffer,
    ...
    if (result.status_code_ != status_list::ok) { return unpack_result_t(result.status_code_); }
    ...
    auto status = unpack_prle_kernel_(
            &unpack_source_ptr, static_cast<uint32_t>(std::distance(unpack_source_ptr, decompress_end_)), bit_width_,
            &current_ptr, static_cast<uint32_t>(required_elements), &prle_count_, &prle_value_);
    ...
    if (unpack_source_ptr != (decompress_begin_ + valid_decompressed_bytes)) {
        ...
        current_decompress_ =
                decompress_begin_ + std::distance(unpack_source_ptr, decompress_begin_ + valid_decompressed_bytes);
