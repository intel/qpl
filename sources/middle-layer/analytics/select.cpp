...
static inline auto select(input_stream_t& input_stream, input_stream_t& mask_stream,
                          output_stream_t<array_stream>& output_stream, limited_buffer_t& unpack_buffer,
                          limited_buffer_t& set_buffer, limited_buffer_t& output_buffer,
    ...
    while (!input_stream.is_processed()) {
        ...
        if (processed_elements != 0) {
            ...
            aggregates_callback(output_buffer.data(), processed_elements, &aggregates.min_value_,
                                &aggregates.max_value_, &aggregates.sum_, &aggregates.index_);
        }
    }
...
template <>
auto call_select<execution_path_t::software>(input_stream_t& input_stream, input_stream_t& mask_stream,
                                             output_stream_t<array_stream>& output_stream,
                                             limited_buffer_t& unpack_buffer, limited_buffer_t& set_buffer,
    ...
    if (input_stream.stream_format() == stream_format_t::prle_format) {
        if (input_stream.is_compressed()) {
            status_code =
                    select<analytic_pipeline::inflate_prle>(input_stream, mask_stream, output_stream, unpack_buffer,
                                                            set_buffer, output_buffer, aggregates_callback, aggregates);
        } else {
            status_code = select<analytic_pipeline::prle>(input_stream, mask_stream, output_stream, unpack_buffer,
                                                          set_buffer, output_buffer, aggregates_callback, aggregates);
        }
    } else {
        if (input_stream.is_compressed()) {
            status_code =
                    select<analytic_pipeline::inflate>(input_stream, mask_stream, output_stream, unpack_buffer,
                                                       set_buffer, output_buffer, aggregates_callback, aggregates);
        } else {
            status_code = select<analytic_pipeline::simple>(input_stream, mask_stream, output_stream, unpack_buffer,
                                                            set_buffer, output_buffer, aggregates_callback, aggregates);
        }
    }
...
template <>
auto call_select<execution_path_t::hardware>(input_stream_t& input_stream, input_stream_t& mask_stream,
                                             output_stream_t<array_stream>& output_stream,
    ...
    descriptor_builder<qpl_op_select>(&completion_record, &aecs_analytic)
...
template <>
auto call_select<execution_path_t::auto_detect>(input_stream_t& input_stream, input_stream_t& mask_stream,
                                                output_stream_t<array_stream>& output_stream,
                                                limited_buffer_t& unpack_buffer, limited_buffer_t& set_buffer,
                                                limited_buffer_t& output_buffer, int32_t numa_id) noexcept
        -> analytic_operation_result_t {
    auto hw_result = call_select<execution_path_t::hardware>(input_stream, mask_stream, output_stream, unpack_buffer,
                                                             set_buffer, output_buffer, numa_id);

    if (hw_result.status_code_ != status_list::ok) {
        return call_select<execution_path_t::software>(input_stream, mask_stream, output_stream, unpack_buffer,
                                                       set_buffer, output_buffer);
    }
