...
public:
    ...
    descriptor_builder(HW_PATH_VOLATILE hw_completion_record* const completion_record_ptr,
        ...
        : completion_record_ptr_(completion_record_ptr), aecs_ptr_(aecs_ptr) {
    ...
    inline auto operation(const input_stream_t& set_stream, uint32_t drop_low_bits, uint32_t drop_high_bits) noexcept
            -> descriptor_builder&;
    ...
    inline auto decompression(const qpl_decomp_end_proc decompression_rule,
        ...
        hw_iaa_descriptor_set_inflate_stop_check_rule(
                &descriptor_, static_cast<hw_iaa_decompress_start_stop_rule_t>(decompression_rule),
                decompression_rule & qpl_check_on_nonlast_block);
        return *this;
    }

    inline auto input(input_stream_t& input_stream) noexcept -> descriptor_builder& {
        hw_iaa_descriptor_analytic_set_filter_input(
                &descriptor_, input_stream.data(), input_stream.size(), input_stream.elements_left(),
                static_cast<hw_iaa_input_format>(input_stream.stream_format()), input_stream.bit_width());
        ...
        if (input_stream.is_compressed()) {
            ...
            hw_iaa_descriptor_set_inflate_stop_check_rule(
                    &descriptor_, static_cast<hw_iaa_decompress_start_stop_rule_t>(meta.end_processing_style), false);
    ...
    inline auto output(const output_stream_t<stream_type>& output_stream) noexcept -> descriptor_builder& {
        ...
        hw_iaa_descriptor_analytic_set_filter_output(&descriptor_, output_stream.data(),
                                                     output_stream.bytes_available(), out_format);
...
template <>
inline auto descriptor_builder<qpl_op_scan_eq>::operation(const uint32_t param_low, const uint32_t param_high) noexcept
        -> descriptor_builder& {
...
template <>
inline auto descriptor_builder<qpl_op_extract>::operation(const uint32_t param_low, const uint32_t param_high) noexcept
        -> descriptor_builder& {
...
template <>
inline auto descriptor_builder<qpl_op_expand>::operation(const input_stream_t& mask_stream) noexcept
        -> descriptor_builder& {
    hw_iaa_descriptor_analytic_set_expand_operation(&descriptor_, mask_stream.data(), mask_stream.size(),
...
template <>
inline auto descriptor_builder<qpl_op_select>::operation(const input_stream_t& mask_stream) noexcept
        -> descriptor_builder& {
    hw_iaa_descriptor_analytic_set_select_operation(&descriptor_, mask_stream.data(), mask_stream.size(),
                                                    mask_stream.stream_format() == stream_format_t::be_format);
