...
static inline auto expand(input_stream_t& input_stream, input_stream_t& mask_stream,
                          limited_buffer_t& unpack_source_buffer, limited_buffer_t& unpack_mask_buffer,
                          limited_buffer_t& output_buffer, output_stream_t<array_stream>& output_stream,
    ...
    while (input_stream.total_elements_count() > output_stream.elements_written() &&
        ...
        if (source_elements == 0) {
            if (input_stream.elements_left() == 0) { return status_list::source_is_short_error; }
        ...
        if (mask_elements == 0) {
            if (mask_stream.elements_left() == 0) { return status_list::source_2_is_short_error; }
        ...
        auto source_elements_used =
                expand_impl(source_ptr, source_elements, mask_ptr, &mask_elements_left, output_buffer.data());
        ...
        if (status_list::ok != pack_status) { return pack_status; }
        aggregates_callback(output_buffer.data(), mask_elements_used, &aggregates.min_value_, &aggregates.max_value_,
...
template <>
auto call_expand<execution_path_t::software>(input_stream_t& input_stream, input_stream_t& mask_stream,
                                             output_stream_t<array_stream>& output_stream,
                                             limited_buffer_t&              unpack_source_buffer,
                                             limited_buffer_t& unpack_mask_buffer, limited_buffer_t& output_buffer,
    ...
    if (input_stream.stream_format() == stream_format_t::prle_format) {
        if (input_stream.is_compressed()) {
            status_code = expand<analytic_pipeline::inflate_prle>(input_stream, mask_stream, unpack_source_buffer,
                                                                  unpack_mask_buffer, output_buffer, output_stream,
                                                                  aggregates_callback, aggregates);
        } else {
            status_code =
                    expand<analytic_pipeline::prle>(input_stream, mask_stream, unpack_source_buffer, unpack_mask_buffer,
                                                    output_buffer, output_stream, aggregates_callback, aggregates);
        }
    } else {
        if (input_stream.is_compressed()) {
            status_code = expand<analytic_pipeline::inflate>(input_stream, mask_stream, unpack_source_buffer,
                                                             unpack_mask_buffer, output_buffer, output_stream,
                                                             aggregates_callback, aggregates);
        } else {
            status_code = expand<analytic_pipeline::simple>(input_stream, mask_stream, unpack_source_buffer,
                                                            unpack_mask_buffer, output_buffer, output_stream,
                                                            aggregates_callback, aggregates);
        }
    }
    ...
    operation_result.last_bit_offset_ =
            (1U == output_stream.bit_width()) ? input_stream.elements_left() & max_bit_index : 0U;
...
template <>
auto call_expand<execution_path_t::hardware>(input_stream_t& input_stream, input_stream_t& mask_stream,
                                             output_stream_t<array_stream>& output_stream,
    ...
    descriptor_builder<qpl_op_expand>(&completion_record, &aecs_analytic)
            .operation(mask_stream)
            .input(input_stream)
...
template <>
auto call_expand<execution_path_t::auto_detect>(input_stream_t& input_stream, input_stream_t& mask_stream,
                                                output_stream_t<array_stream>& output_stream,
                                                limited_buffer_t&              unpack_source_buffer,
                                                limited_buffer_t& unpack_mask_buffer, limited_buffer_t& output_buffer,
                                                int32_t numa_id) noexcept -> analytic_operation_result_t {
    auto hw_result = call_expand<execution_path_t::hardware>(
            input_stream, mask_stream, output_stream, unpack_source_buffer, unpack_mask_buffer, output_buffer, numa_id);

    if (hw_result.status_code_ != status_list::ok) {
        return call_expand<execution_path_t::software>(input_stream, mask_stream, output_stream, unpack_source_buffer,
                                                       unpack_mask_buffer, output_buffer);
    }
