...
static uint64_t crc64_init_crc(uint64_t polynomial, bool is_big_endian, bool is_inverse) {
    ...
    if (!is_big_endian) { return polynomial; }
...
static uint64_t crc64_finalize(uint64_t crc, uint64_t polynomial, bool is_big_endian, bool is_inverse) {
    ...
    polynomial |= (polynomial << 16);
    polynomial |= (polynomial << 32);
    if (!is_big_endian) { return crc ^ polynomial; }

    return crc ^ bit_byte_swap_64(polynomial);
}

auto perform_crc(const uint8_t* src_ptr, uint32_t length, uint64_t polynomial, bool is_be_bit_order, bool is_inverse)
        -> uint64_t {
    uint64_t table[256];
...
template <>
auto call_crc<execution_path_t::hardware>(const uint8_t* src_ptr, uint32_t length, uint64_t polynomial,
                                          bool is_be_bit_order, bool is_inverse, int32_t numa_id) noexcept
        -> crc_operation_result_t {
    ...
    hw_iaa_descriptor_init_crc64(&descriptor, src_ptr, length, polynomial, is_be_bit_order, is_inverse);

    return util::process_descriptor<crc_operation_result_t, util::execution_mode_t::sync>(&descriptor,
                                                                                          &completion_record, numa_id);
}
...
template <>
auto call_crc<execution_path_t::software>(const uint8_t* src_ptr, uint32_t length, uint64_t polynomial,
    ...
    const uint32_t         status_code = status_list::ok;
...
template <>
auto call_crc<execution_path_t::auto_detect>(const uint8_t* src_ptr, uint32_t length, uint64_t polynomial,
                                             bool is_be_bit_order, bool is_inverse, int32_t numa_id) noexcept
        -> crc_operation_result_t {
    auto hw_result =
            call_crc<execution_path_t::hardware>(src_ptr, length, polynomial, is_be_bit_order, is_inverse, numa_id);

    if (hw_result.status_code_ != status_list::ok) {
        return call_crc<execution_path_t::software>(src_ptr, length, polynomial, is_be_bit_order, is_inverse);
    }
