...
static inline void cases_set(data_t& data, crc_type_e type) {
    if (continue_register(execution_e::sync))
        register_benchmarks_common("crc64", to_name(path, "gen_path") + crc_to_name(type),
                                   crc64_t<execution_e::sync, api_e::c, path> {}, case_params_t {}, data, type);

    if (continue_register(execution_e::async))
        register_benchmarks_common("crc64", to_name(path, "gen_path") + crc_to_name(type),
                                   crc64_t<execution_e::async, api_e::c, path> {}, case_params_t {}, data, type);
}

BENCHMARK_SET_DELAYED(crc64) {
    if (!continue_register(bench::operation_e::crc64)) return;
        ...
        for (auto& size : block_sizes) {
            ...
            for (auto& block : blocks) {
                for (auto& crc_type : crc_types) {
                    if (continue_register(path_e::iaa)) cases_set<path_e::iaa>(block, crc_type);

                    if (continue_register(path_e::cpu)) cases_set<path_e::cpu>(block, crc_type);
                }
            }
        }
    }
