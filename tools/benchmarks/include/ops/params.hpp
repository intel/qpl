...
struct deflate_params_t
{
    ...
    deflate_params_t(const data_t &data, std::int32_t level, huffman_type_e huffman, bool huffman_only = false, bool no_headers = false, std::shared_ptr<qpl_huffman_table> huffman_table = nullptr) :
        p_source_data_(&data),
        level_(level),
        huffman_(huffman),
...
struct inflate_params_t
{
    ...
    inflate_params_t(const data_t &stream, size_t original_size, bool no_headers, huffman_type_e mode = huffman_type_e::fixed, std::shared_ptr<qpl_huffman_table> huffman_table = nullptr) :
        p_stream_(&stream),
