/*******************************************************************************
 * Copyright (C) 2022 Intel Corporation
 *
 * SPDX-License-Identifier: MIT
 ******************************************************************************/

/**
 * @date 10/30/2018
 * Contains an implementation of the @ref ref_crc32 function
 */

#include "qpl_api_ref.h"

/**
 * @defgroup REFERENCE_CRC32 CRC-32
 * @ingroup REFERENCE_PRIVATE
 * @{
 * @brief Contains helper functions for the @ref ref_crc32
 */

// clang-format off

/**
 * @brief Auxiliary table to perform bits reversing in byte
 */
static const uint8_t bit_reverse_table_8[0x100] = {
        ,
};

// clang-format on

/**
 * @brief Helper for reversing bits in byte
 * @todo
 *
 */
static inline uint8_t reverse_bits_8(uint8_t x) {
    return bit_reverse_table_8[x];
}

/**
 * @brief Helper for reversing bits in uint32_t
 * @todo
 *
 */
static inline uint32_t reverse_bits_32(uint32_t x) {
    uint32_t y = 0;

    y = reverse_bits_8(x >> 24);
    y |= reverse_bits_8(x >> 16) << 8;
    y |= reverse_bits_8(x >> 8) << 16;
    y |= reverse_bits_8(x >> 0) << 24;

    return y;
}

/** @} */

uint32_t ref_crc32(const uint8_t* buf, uint32_t len, uint32_t poly, uint32_t init_crc) {
    uint32_t crc = ~init_crc;

    for (uint32_t i = 0; i < len; i++) {
        crc ^= (uint32_t)(reverse_bits_8(buf[i])) << 24;

        for (uint32_t j = 0; j < 8; j++) {
            crc = (0x80000000 & crc) ? (crc << 1) ^ poly : crc << 1;
        }
    }

    return ~reverse_bits_32(crc);
}
