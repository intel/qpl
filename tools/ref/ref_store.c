 ...
 * @return
 */
REF_INLINE qpl_status own_store_value_8(const uint32_t* const source_ptr, uint32_t number_of_elements,
                                        uint32_t source_bit_width, uint8_t* const destination_ptr,
                                        const uint8_t* const destination_end_ptr, uint32_t* element_index_ptr);
 ...
 * @return
 */
REF_INLINE qpl_status own_store_value_16(const uint32_t* const source_ptr, uint32_t number_of_elements,
                                         uint32_t source_bit_width, uint8_t* const destination_ptr,
                                         const uint8_t* const destination_end_ptr, bool output_be,
 ...
 * @return
 */
REF_INLINE qpl_status own_store_value_32(const uint32_t* const source_ptr, uint32_t number_of_elements,
                                         uint32_t source_bit_width, uint8_t* const destination_ptr,
                                         const uint8_t* const destination_end_ptr, bool output_be,
                                         uint32_t* element_index_ptr);
 ...
 * @return
 */
REF_INLINE qpl_status own_store_value_nom(const uint32_t* const source_ptr, uint32_t number_of_elements,
                                          uint32_t source_bit_width, uint8_t* const destination_ptr, bool output_be,
                                          uint32_t* element_index_ptr);
...
qpl_status ref_store_values(const uint32_t* const source_ptr, uint32_t number_of_elements, uint32_t source_bit_width,
    ...
    switch (output_format) {
        case qpl_ow_8: {
            return own_store_value_8(source_ptr, number_of_elements, source_bit_width, destination_ptr,
                                     destination_end_ptr, element_index_ptr);
        }
        case qpl_ow_16: {
            return own_store_value_16(source_ptr, number_of_elements, source_bit_width, destination_ptr,
                                      destination_end_ptr, output_be, element_index_ptr);
        }
        case qpl_ow_32: {
            return own_store_value_32(source_ptr, number_of_elements, source_bit_width, destination_ptr,
                                      destination_end_ptr, output_be, element_index_ptr);
        }
        case qpl_ow_nom: {
            return own_store_value_nom(source_ptr, number_of_elements, source_bit_width, destination_ptr, output_be,
                                       element_index_ptr);
        }
        default: {
...
REF_INLINE qpl_status own_store_value_8(const uint32_t* const source_ptr, uint32_t number_of_elements,
    ...
    for (uint32_t i = 0U; i < number_of_elements; ++i) {
        ...
        if (QPL_ONE_32U == source_bit_width) {
            if (0U < value) {
                if (UINT8_MAX < index) { return QPL_STS_OUTPUT_OVERFLOW_ERR; }
...
REF_INLINE qpl_status own_store_value_16(const uint32_t* const source_ptr, uint32_t number_of_elements,
    ...
    for (uint32_t i = 0U; i < number_of_elements; ++i) {
        ...
        if (QPL_ONE_32U == source_bit_width) {
            if (0U < value) {
                if (UINT16_MAX < index) { return QPL_STS_OUTPUT_OVERFLOW_ERR; }
    ...
    if (output_be) {
        ...
        for (uint32_t i = 0U; i < number_of_elements; i++) {
            ...
            for (uint32_t j = 0U; j < source_bit_width; j++) {
                ...
                // Store bit
                (*current_destination_ptr) = (sourceBit) ? ((*current_destination_ptr) | destination_mask)
