 ...
 * @return
 */
REF_INLINE qpl_status own_select(const uint32_t *const source_ptr,
 ...
 * @return
 */
REF_INLINE qpl_status own_select_output_to_format(const uint32_t *const source_ptr,
                                                  uint32_t number_of_elements,
...
REF_INLINE qpl_status own_select_prle(qpl_job *const qpl_job_ptr) {
    ...
    // Main action
    status = own_select(extracted_ptr,
                        QPL_MIN(number_of_elements, source_length),
                        extracted_mask_ptr,
                        results_ptr,
                        &destination_length);
