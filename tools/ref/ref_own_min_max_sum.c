...
qpl_status ref_min_max_sum(const uint8_t *src_ptr,
    ...
    // input len in bits
    uint64_t src_bit_len    = last_bit_offset > 0
                              ? ((len - 1) * REF_BYTE_BIT_LEN) + last_bit_offset
                              : len * REF_BYTE_BIT_LEN;
    ...
    for (uint32_t curr_elem_ind = 0; curr_elem_ind < num_of_elements; curr_elem_ind++) {
        ...
        if (1 == elem_bit_width) {
            ...
            sum_value += elem;
#ifdef OWN_SATURATION_ENABLED
            if(UINT32_MAX < sum_value)
            {// check for uint32_t overflow and perform saturation
                sum_value = UINT32_MAX;
            }
#endif

            if (elem > last_index_max_value) {
                last_index_max_value = elem;
            }

            if (elem < first_index_min_value) {
                first_index_min_value = elem;
            }
        }
    }
