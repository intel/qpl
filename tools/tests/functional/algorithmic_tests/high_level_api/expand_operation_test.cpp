...
QPL_HIGH_LEVEL_API_ALGORITHMIC_TEST(expand, little_endian) {
    ...
    for (auto number_of_elements : source_length) {
        ...
        for (uint32_t output_bit_width : {1, 8, 16, 32}) {
            for (uint32_t input_bit_width = 1; input_bit_width <= max_input_bit_width; input_bit_width++) {
                ...
                auto expand_operation = typename expand_operation::builder(mask.data(), mask.size())
                ...
                ASSERT_NO_THROW(auto expand_operation_result = test::execute(expand_operation, source, destination);
                                        result_elements_count = handle_result(expand_operation_result))
                                            << test_case_info;
                ...
                auto status = ref_expand(reference_job_ptr);
...
QPL_HIGH_LEVEL_API_ALGORITHMIC_TEST(expand, big_endian) {
...
QPL_HIGH_LEVEL_API_ALGORITHMIC_TEST(expand, parquet_rle) {
