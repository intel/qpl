...
namespace qpl::test
{
    ...
    public:
        void SetUpBeforeIteration() override
        {
            ...
            source_provider source_generator(current_test_case.number_of_elements,
    ...
    QPL_LOW_LEVEL_API_ALGORITHMIC_TEST_TC(aggregates, min_max_sum, MinMaxSumTest)
    {
        ...
        ref_min_max_sum(destination.data(),
                     job_ptr->total_out,
                     job_ptr->last_bit_offset,
                     job_ptr->out_bit_width,
                     1,
                     job_ptr->flags & QPL_FLAG_OUT_BE,
                     0,
                     &reference_min_value,
                     &reference_max_value,
                     &reference_sum_value);
