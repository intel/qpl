...
size_t calculate_table_size(const uint32_t mini_block_count, const uint32_t mini_blocks_per_block) {
    ...
    const uint32_t blocks_in_deflate_count = (mini_block_count + mini_blocks_per_block - 1U) / mini_blocks_per_block;
...
QPL_LOW_LEVEL_API_ALGORITHMIC_TEST(index_table, DISABLED_set_mini_block_location) {
    ...
    *next_in_pptr += start_bit / 8;
    ...
    auto status = qpl_set_mini_block_location(start_bit, last_bit, source_pptr, first_bit_offset_ptr,
                                              last_bit_offset_ptr, compressed_size_ptr);
...
QPL_LOW_LEVEL_API_ALGORITHMIC_TEST(index_table, find_header_block_index) {
    ...
    ASSERT_EQ(table.mini_block_count, table.block_count * table.mini_blocks_per_block)
            << "Incompatible mini_block_count";
