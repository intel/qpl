    ...
    void InitializeTestCases() {
        ...
        for (const uint32_t length : lengths) {
            for (uint32_t bit_width = 1; bit_width <= 32U; bit_width++) {
                for (const uint32_t destination_bit_width : {1, 8, 16, 32}) {
...
public:
    ...
    void SetBuffers() override {
        ...
        if (aligned_src_buffer == nullptr || aligned_dst_buffer == nullptr || aligned_src2_buffer == nullptr) {
        ...
        aligned_src2 = std::unique_ptr<uint8_t[], void (*)(void*)>(aligned_src2_buffer, std::free);
