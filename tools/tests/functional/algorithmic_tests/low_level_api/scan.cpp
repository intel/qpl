...
namespace qpl::test
{
    ...
    QPL_LOW_LEVEL_API_ALGORITHMIC_TEST_TC(scan, force_array_output_modification, ScanTest)
    {
        ...
        job->out_bit_width      = qpl_ow_32;  // without Force Array Output Mod, this will causes scan to return an array of indices
        ...
        for (uint32_t i = 0; i < source_size; ++i) {
            ...
            ASSERT_EQ((1UL << i) & reference ? 1U : 0U, output[i]) << "Output value at index " << i << " does not match the expected value.\n";
        }
    }
