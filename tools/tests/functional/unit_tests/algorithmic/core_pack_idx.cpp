...
static uint32_t ref_qplc_pack_index_8u(const uint8_t* src_ptr, uint32_t num_elements,
    uint8_t** pp_dst, uint32_t dst_length, uint32_t* index_ptr)
{
...
static uint32_t ref_qplc_pack_index_8u16u(const uint8_t* src_ptr, uint32_t num_elements,
    uint8_t** pp_dst, uint32_t dst_length, uint32_t* index_ptr)
{
    uint32_t    index = *index_ptr;
    uint32_t    status = QPLC_STS_OK;
...
static uint32_t ref_qplc_pack_index_8u32u(const uint8_t* src_ptr, uint32_t num_elements,
    uint8_t** pp_dst, uint32_t dst_length, uint32_t* index_ptr)
{
...
constexpr uint32_t fun_indx_pack_idx_8u     = 1;
constexpr uint32_t fun_indx_pack_idx_8u16u  = 2;
constexpr uint32_t fun_indx_pack_idx_8u32u  = 3;
...
QPL_UNIT_API_ALGORITHMIC_TEST(qplc_pack_idx_8u, base) {
    ...
    for (uint32_t length = 1; length <= TEST_BUFFER_SIZE; length++) {
        for (uint32_t length_dst = 1; length_dst <= TEST_BUFFER_SIZE; length_dst++) {
            ...
            status = (uint32_t)qplc_pack_vector(fun_indx_pack_idx_8u)(source.data(), length, &p_dst_8u, length_dst, &index);
            ref_status = (uint32_t)ref_qplc_pack_index_8u(source.data(), length, &p_ref_dst_8u, length_dst, &ref_index);
            ...
            ASSERT_TRUE(CompareSegments(reference.begin(), reference.end(),
                destination.begin(), destination.end(), "FAIL qplc_pack_idx_8u!!! "));
        }
        for (uint32_t length = 1; length <= TEST_BUFFER_SIZE; length++) {
            for (uint32_t length_dst = 1; length_dst <= TEST_BUFFER_SIZE; length_dst++) {
                destination.fill(0);
                ...
                uint32_t status;
                uint32_t ref_status;
                status = (uint32_t)qplc_pack_vector(fun_indx_pack_idx_8u)(source.data(), length,
                    &p_dst_16u, length_dst, &index);
                ref_status = (uint32_t)ref_qplc_pack_index_8u(source.data(), length,
...
QPL_UNIT_API_ALGORITHMIC_TEST(qplc_pack_idx_8u16u, base) {
    ...
    for (uint32_t length = 1; length <= TEST_BUFFER_SIZE; length++) {
        for (uint32_t length_dst = 1; length_dst <= TEST_BUFFER_SIZE; length_dst++) {
            ...
            status = (uint32_t)qplc_pack_vector(fun_indx_pack_idx_8u16u)(source.data(), length, &p_dst_8u, length_dst, &index);
            ref_status = (uint32_t)ref_qplc_pack_index_8u16u(source.data(), length, &p_ref_dst_8u, length_dst, &ref_index);
            ASSERT_EQ(status, ref_status);
            ASSERT_EQ(index, ref_index);
            ASSERT_EQ((p_dst_8u - (uint8_t*)destination.data()), (p_ref_dst_8u - (uint8_t*)reference.data()));
            ASSERT_TRUE(std::equal(reference.begin(), reference.end(), destination.data()));
            ASSERT_TRUE(CompareSegments(reference.begin(), reference.end(),
                destination.begin(), destination.end(), "FAIL qplc_pack_idx_8u16u!!! "));
        }
    }
    for (uint32_t length = 1; length <= TEST_BUFFER_SIZE; length++) {
        for (uint32_t length_dst = 1; length_dst <= TEST_BUFFER_SIZE; length_dst++) {
...
QPL_UNIT_API_ALGORITHMIC_TEST(qplc_pack_idx_8u32u, base) {
    ...
    for (uint32_t length = 1; length <= TEST_BUFFER_SIZE; length++) {
        for (uint32_t length_dst = 1; length_dst <= TEST_BUFFER_SIZE; length_dst++) {
            ...
            status = (uint32_t)qplc_pack_vector(fun_indx_pack_idx_8u32u)(source.data(), length, &p_dst_8u, length_dst, &index);
            ref_status = (uint32_t)ref_qplc_pack_index_8u32u(source.data(), length, &p_ref_dst_8u, length_dst, &ref_index);
            ASSERT_EQ(status, ref_status);
            ASSERT_EQ(index, ref_index);
            ASSERT_EQ((p_dst_8u - (uint8_t*)destination.data()), (p_ref_dst_8u - (uint8_t*)reference.data()));
            ASSERT_TRUE(CompareSegments(reference.begin(), reference.end(),
                destination.begin(), destination.end(), "FAIL qplc_pack_idx_8u32u!!! "));
        }
        for (uint32_t length = 1; length <= TEST_BUFFER_SIZE; length++) {
            for (uint32_t length_dst = 1; length_dst <= TEST_BUFFER_SIZE; length_dst++) {
                destination.fill(0);
