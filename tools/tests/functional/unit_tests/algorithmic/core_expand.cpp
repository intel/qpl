...
constexpr uint32_t fun_indx_expand_8u = 0;
constexpr uint32_t fun_indx_expand_16u = 1;
constexpr uint32_t fun_indx_expand_32u = 2;
...
QPL_UNIT_API_ALGORITHMIC_TEST(qplc_expand_8u, base) {
    ...
    for (uint32_t length = 1; length <= TEST_BUFFER_SIZE; length++) {
        ...
        uint32_t expanded;
        uint32_t ref_expanded;

        destination.fill(0);
        reference.fill(0);
        expanded = qplc_expand(0)(source.data(), length, mask.data(), &length2, destination.data());
        ref_expanded = ref_qplc_expand_8u(source.data(), length, mask.data(),&ref_length2, reference.data());
...

    {
        ...
        for (uint32_t indx = 0; indx < 16; indx++) {
            p_buffer_mask[indx] = 0u;
        }
    }

    for (uint32_t length = 16; length <= TEST_BUFFER_SIZE; length++) {
        uint32_t length2 = length;
        uint32_t ref_length2 = length;
        uint32_t expanded;
        uint32_t ref_expanded;
    ...
    for (uint32_t length = 2; length <= TEST_BUFFER_SIZE; length++) {
        uint32_t length2 = length;
        uint32_t ref_length2 = length;
        uint32_t expanded;
        uint32_t ref_expanded;
        ...
        destination.fill(0);
        reference.fill(0);
        expanded = qplc_expand(0)(source.data(), length - 1, mask.data(), &length2, destination.data());
        ref_expanded = ref_qplc_expand_8u(source.data(), length - 1, mask.data(), &ref_length2, reference.data());
        ASSERT_TRUE(CompareSegments(reference.begin(), reference.end(),
            destination.begin(), destination.end(), "FAIL qplc_expand_8u!!! "));
        ASSERT_EQ(length2, ref_length2);
        ASSERT_EQ(expanded, ref_expanded);
    ...
    {
        uint8_t* p_buffer_mask = (uint8_t*)mask.data();
        for (uint32_t indx = 0; indx < 32; indx++) {
            p_buffer_mask[indx] = 0u;
        }
    }

    for (uint32_t length = 33; length <= TEST_BUFFER_SIZE; length++) {
        uint32_t length2 = length;
        uint32_t ref_length2 = length;
        uint32_t expanded;
        uint32_t ref_expanded;
        ...
        destination.fill(0);
        reference.fill(0);
        expanded = qplc_expand(0)(source.data(), length - 32, mask.data(), &length2, destination.data());
        ref_expanded = ref_qplc_expand_8u(source.data(), length - 32, mask.data(), &ref_length2, reference.data());
        ASSERT_TRUE(CompareSegments(reference.begin(), reference.end(),
            destination.begin(), destination.end(), "FAIL qplc_expand_8u!!! "));
        ASSERT_EQ(length2, ref_length2);
        ASSERT_EQ(expanded, ref_expanded);
...
QPL_UNIT_API_ALGORITHMIC_TEST(qplc_expand_16u, base) {
    ...
    for (uint32_t length = 1; length <= TEST_BUFFER_SIZE; length++) {
        ...
        destination.fill(0);
        reference.fill(0);
        expanded = qplc_expand(1)(source.data(), length, mask.data(), &length2, destination.data());
        ref_expanded = ref_qplc_expand_16u(source.data(), length, mask.data(), &ref_length2, reference.data());
        ASSERT_TRUE(CompareSegments(reference.begin(), reference.end(),
    ...
    for (uint32_t length = 2; length <= TEST_BUFFER_SIZE; length++) {
        ...
        destination.fill(0);
        reference.fill(0);
        expanded = qplc_expand(1)(source.data(), length - 1, mask.data(), &length2, destination.data());
        ref_expanded = ref_qplc_expand_16u(source.data(), length - 1, mask.data(), &ref_length2, reference.data());
        ASSERT_TRUE(CompareSegments(reference.begin(), reference.end(),
            destination.begin(), destination.end(), "FAIL qplc_expand_16u!!! "));
        ASSERT_EQ(length2, ref_length2);
        ASSERT_EQ(expanded, ref_expanded);
    ...
    for (uint32_t length = 33; length <= TEST_BUFFER_SIZE; length++) {
        ...
        destination.fill(0);
        reference.fill(0);
        expanded = qplc_expand(1)(source.data(), length - 32, mask.data(), &length2, destination.data());
        ref_expanded = ref_qplc_expand_16u(source.data(), length - 32, mask.data(), &ref_length2, reference.data());
        ASSERT_TRUE(CompareSegments(reference.begin(), reference.end(),
            destination.begin(), destination.end(), "FAIL qplc_expand_16u!!! "));
        ASSERT_EQ(length2, ref_length2);
        ASSERT_EQ(expanded, ref_expanded);
...
QPL_UNIT_API_ALGORITHMIC_TEST(qplc_expand_32u, base) {
    ...
    for (uint32_t length = 1; length <= TEST_BUFFER_SIZE; length++) {
        ...
        destination.fill(0);
        reference.fill(0);
        expanded = qplc_expand(2)(source.data(), length, mask.data(), &length2, destination.data());
        ref_expanded = ref_qplc_expand_32u(source.data(), length, mask.data(), &ref_length2, reference.data());
        ASSERT_TRUE(CompareSegments(reference.begin(), reference.end(),
    ...
    for (uint32_t length = 2; length <= TEST_BUFFER_SIZE; length++) {
        ...
        destination.fill(0);
        reference.fill(0);
        expanded = qplc_expand(2)(source.data(), length - 1, mask.data(), &length2, destination.data());
        ref_expanded = ref_qplc_expand_32u(source.data(), length - 1, mask.data(), &ref_length2, reference.data());
        ASSERT_TRUE(CompareSegments(reference.begin(), reference.end(),
            destination.begin(), destination.end(), "FAIL qplc_expand_32u!!! "));
        ASSERT_EQ(length2, ref_length2);
        ASSERT_EQ(expanded, ref_expanded);
    ...
    for (uint32_t length = 33; length <= TEST_BUFFER_SIZE; length++) {
        ...
        destination.fill(0);
        reference.fill(0);
        expanded = qplc_expand(2)(source.data(), length - 32, mask.data(), &length2, destination.data());
        ref_expanded = ref_qplc_expand_32u(source.data(), length - 32, mask.data(), &ref_length2, reference.data());
        ASSERT_TRUE(CompareSegments(reference.begin(), reference.end(),
            destination.begin(), destination.end(), "FAIL qplc_expand_32u!!! "));
        ASSERT_EQ(length2, ref_length2);
        ASSERT_EQ(expanded, ref_expanded);
