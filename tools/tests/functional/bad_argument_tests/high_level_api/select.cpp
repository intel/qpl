...
QPL_HIGH_LEVEL_API_BAD_ARGUMENT_TEST(select, zero_destination_size) {
    ...
    auto select_operation = qpl::select_operation(mask.data(), mask.size());

    auto status = test::get_execute_status(select_operation,
                                           source.begin(),
                                           source.end(),
                                           destination.begin(),
                                           destination.end());
...
QPL_HIGH_LEVEL_API_BAD_ARGUMENT_TEST(select, zero_mask_size) {
    ...
    auto select_operation = qpl::select_operation::builder(mask.data(), mask.size())
            .build();
...
QPL_HIGH_LEVEL_API_BAD_ARGUMENT_TEST(select, small_mask_size) {
    constexpr const uint32_t zero_mask_size = 0;
    ...
    EXPECT_EQ(status, QPL_STS_SRC2_IS_SHORT_ERR) << "Fail on: mask bit size should be at least number_of_input_elements";
}

QPL_HIGH_LEVEL_API_BAD_ARGUMENT_TEST(select, incorrect_source_size) {
...
QPL_HIGH_LEVEL_API_BAD_ARGUMENT_TEST(select, incorrect_bit_width) {
    std::vector<uint8_t> source(source_size);
    std::vector<uint8_t> destination(destination_size);
    std::vector<uint8_t> mask(destination_size);

    ASSERT_THROW(auto select_operation = qpl::select_operation::builder(mask.data(), mask.size())
            .input_vector_width(0)
            .build(), qpl::invalid_argument_exception);
}

QPL_HIGH_LEVEL_API_BAD_ARGUMENT_TEST(select, incorrect_prle_input_bit_width) {
...
QPL_HIGH_LEVEL_API_BAD_ARGUMENT_TEST(select, buffer_overlap) {
    ...
    auto select_operation = qpl::select_operation::builder(mask_begin, mask_size_)
            .parser<qpl::parsers::little_endian_packed_array>(source_size)
            .input_vector_width(input_bit_width)
            .build();
    ...
    status = test::get_execute_status(select_operation,
                                      source_begin,
                                      source_begin + source_size,
                                      destination_begin,
                                      destination_begin + destination_size);
    ...
    select_operation = qpl::select_operation::builder(mask_begin, mask_size_)
            .parser<qpl::parsers::little_endian_packed_array>(source_size)
            .input_vector_width(input_bit_width)
            .build();
