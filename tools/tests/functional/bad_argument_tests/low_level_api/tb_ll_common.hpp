...
enum class operation_group_e {
    filter_single_source,
    filter_double_source
} ;

void set_input_stream(qpl_job *job_ptr,
                      uint8_t *source_ptr,
                      uint32_t source_size,
                      uint32_t input_bit_width,
                      uint32_t elements_to_process,
                      qpl_parser parser);

void set_mask_stream(qpl_job *job_ptr,
                     uint8_t *mask_ptr,
                     uint32_t mask_size,
                     uint32_t mask_bit_width);

void set_output_stream(qpl_job *job_ptr,
                       uint8_t *destination_ptr,
                       uint32_t destination_size,
                       qpl_out_format output_bit_width);

void set_operation_properties(qpl_job *job_ptr,
...
static inline void check_double_source_stream_limits_validation(qpl_job *const job_ptr,
                                                                qpl_operation operation,
                                                                flags_t flags,
...
static inline void check_initial_output_index_verification(qpl_job *const job_ptr, qpl_operation operation, flags_t flags)
{
    std::array<uint8_t, SOURCE_ARRAY_SIZE>      source{};
    std::array<uint8_t, MASK_ARRAY_SIZE>        mask{};
    std::array<uint8_t, DESTINATION_ARRAY_SIZE> destination{};
...
static inline void check_drop_initial_bytes_verification(qpl_job *const job_ptr, qpl_operation operation, flags_t flags)
{
    ...
    if constexpr(group == operation_group_e::filter_single_source) {
        ...
        if (!(flags & QPL_FLAG_DECOMPRESS_ENABLE)) {
            ...
            EXPECT_EQ(qpl::test::run_job_api(job_ptr), QPL_STS_DROP_BYTES_ERR) << "Fail on: drop initial bytes > avail_in";
            job_ptr->drop_initial_bytes = DROP_INITIAL_BYTES;
...
static inline void analytic_with_mask_check_num_elements_with_index_output(qpl_job *const job_ptr, qpl_operation operation, flags_t flags)
{
    ...
    EXPECT_EQ(qpl::test::run_job_api(job_ptr), QPL_STS_OUTPUT_OVERFLOW_ERR) << "Fail on: number of elements exceeds max index value error (mode: qpl_ow_8)";
    ...
    EXPECT_EQ(qpl::test::run_job_api(job_ptr), QPL_STS_OUTPUT_OVERFLOW_ERR) << "Fail on: number of elements exceeds max index value error (mode: qpl_ow_16)";
}
