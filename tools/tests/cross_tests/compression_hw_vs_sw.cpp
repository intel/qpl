...
namespace qpl::test {
    ...
    protected:
        void SetUp() override {
            ...
            //Set Up compression huffman table
            huffman_table_buffer.resize(QPL_COMPRESSION_TABLE_SIZE);
            std::fill(huffman_table_buffer.begin(), huffman_table_buffer.end(), 0u);
            compression_table_ptr = reinterpret_cast<qpl_compression_huffman_table *>(huffman_table_buffer.data());
            fill_compression_table(compression_table_ptr);
        }
...
*/
        ...
        std::vector<uint8_t> huffman_table_buffer;
    ...
    public:
        ...
        void SetUpBeforeIteration() override {
            ...
            if (current_test_case.block_type == block_static) {
                sw_job_ptr->compression_huffman_table = compression_table_ptr;
                hw_job_ptr->compression_huffman_table = compression_table_ptr;
            } else if (current_test_case.block_type == block_fixed) {
                sw_job_ptr->compression_huffman_table = nullptr;
                hw_job_ptr->compression_huffman_table = nullptr;
            } else {
