...
class SimpleCannedOneChuckCompressDecompressFixture
...
protected:
    ...
    void SetUp() override {
        ...
        auto status = qpl_deflate_huffman_table_create(compression_table_type, qpl_path_auto, DEFAULT_ALLOCATOR_C,
                                                       &c_huffman_table);
        if (status != QPL_STS_OK) { throw std::runtime_error("Compression table creation failed."); }
        ...
        status = qpl_deflate_huffman_table_create(decompression_table_type, qpl_path_auto, DEFAULT_ALLOCATOR_C,
                                                  &d_huffman_table);
        if (status != QPL_STS_OK) { throw std::runtime_error("Decompression table creation failed."); }
    }
...
public:
    SimpleCannedOneChuckCompressDecompressFixture()
        : c_huffman_table(NULL), d_huffman_table(NULL) {

        };
    SimpleCannedOneChuckCompressDecompressFixture(const SimpleCannedOneChuckCompressDecompressFixture&)  = delete;
    SimpleCannedOneChuckCompressDecompressFixture(const SimpleCannedOneChuckCompressDecompressFixture&&) = delete;
    SimpleCannedOneChuckCompressDecompressFixture&
    operator=(const SimpleCannedOneChuckCompressDecompressFixture&) = delete;
    SimpleCannedOneChuckCompressDecompressFixture&
    operator=(const SimpleCannedOneChuckCompressDecompressFixture&&) = delete;
    ...
    testing::AssertionResult ValidateCompressSwDecompressHw(qpl_compression_levels level) {
        init_compression_huffman_table(c_huffman_table, reference_text.data(),
                                       reference_text.data() + static_cast<uint32_t>(reference_text.size()), level,
                                       qpl_path_software);
    ...
    testing::AssertionResult ValidateCompressHwDecompressSw() {
        init_compression_huffman_table(c_huffman_table, reference_text.data(),
                                       ...
                                       qpl_default_level, qpl_path_hardware);
...
QPL_LOW_LEVEL_API_CROSS_TEST_TC(deflate_canned, SimpleCannedOneChuckCompressDecompressFixture,
                                stateless_compress_sw_decompress_hw_default_level) {
    ASSERT_TRUE(ValidateCompressSwDecompressHw(qpl_default_level));
}

QPL_LOW_LEVEL_API_CROSS_TEST_TC(deflate_canned, SimpleCannedOneChuckCompressDecompressFixture,
                                stateless_compress_sw_decompress_hw_high_level) {
    ASSERT_TRUE(ValidateCompressSwDecompressHw(qpl_high_level));
}

QPL_LOW_LEVEL_API_CROSS_TEST_TC(deflate_canned, SimpleCannedOneChuckCompressDecompressFixture,
                                stateless_compress_hw_decompress_sw_default_level) {
